...
## 0. How to read the math layer

This file is the **baseline root spec** for the repository.

It defines the predictive-coding math that remains authoritative unless a later
stage addendum explicitly overrides it inside that addendum's declared scope.

To build a complete mathematical understanding, read in this order:

1. this baseline file
2. if the task touches `stage_03_transport_core_v1/` or later:
   - [specs/stage_03_transport_core_v1.md](/e:/CodeSpace/PredictiveCoding/specs/stage_03_transport_core_v1.md)
3. if the task touches `stage_04_incremental_bridge/`:
...
- the Stage 04 addendum overrides the Stage 03 addendum only inside its declared scope
- the Stage 03 addendum overrides this baseline only inside its declared scope
- outside addendum scope, this baseline file remains authoritative

No addendum should silently redefine untouched baseline equations.
...
## 16. Stage addenda index

The stage-specific mathematical extensions now live in separate addendum files so
that the baseline spec can stay readable while the full FMPC math remains explicit.

Current addenda:

- Stage 03 transport contract:
  - [specs/stage_03_transport_core_v1.md](/e:/CodeSpace/PredictiveCoding/specs/stage_03_transport_core_v1.md)
...
- for Stage 03 work:
  - read this baseline file, then the Stage 03 addendum
- for Stage 04 work:
