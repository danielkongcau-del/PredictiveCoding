...
_SuccessorHandoffMode = Literal[
    "live_successor_handoff",
    "cached_on_successor_only",
    "cached_lf_successor_only",
    "cached_both_successor_components",
]
_Diagnosis = Literal[
    "stale_successor_value_is_primary_blocker",
    "successor_feature_bundle_is_primary_blocker",
    "cross_source_successor_inconsistency_is_primary_blocker",
    "no_single_successor_component_dominates__interaction_blocker",
]
...
class _CaseSpec:
    ...
    successor_handoff_mode: _SuccessorHandoffMode = "live_successor_handoff"


@dataclass
class FMPCTF2SuccessorHandoffSourceSuiteConfig:
    """Run a very narrow successor-handoff source-localization diagnostic."""

    experiment_name: str = "fmpc_tf2_successor_handoff_source_suite"
    ...
    primary_gate_positive_rate_threshold: float = 0.8
    primary_selected_gate_rate_threshold: float = 0.8
    primary_selector_fallback_used_rate_threshold: float = 0.2
    cross_source_accuracy_margin: float = 0.01
    cross_source_rowspace_margin: float = 0.005
    ...
    def case_specs(self) -> tuple[_CaseSpec, ...]:
        return (
            ...
            _CaseSpec(
                ...
                description="Earlier-control failure reference: same adopted full-vector hard 30 degree cone on the penultimate and terminal micro-steps with the full live successor handoff.",
                intervention_step_offsets=(-2, -1),
                successor_handoff_mode="live_successor_handoff",
            ),
            _CaseSpec(
                case_name="cached_on_successor_only",
                description="Swap only the preterminal learned/on-policy successor state z_on_next back to the cached batch-start successor.",
                intervention_step_offsets=(-2, -1),
                successor_handoff_mode="cached_on_successor_only",
            ),
            _CaseSpec(
                case_name="cached_lf_successor_only",
                description="Swap only the preterminal local-field successor state z_lf_next back to the cached batch-start successor.",
                intervention_step_offsets=(-2, -1),
                successor_handoff_mode="cached_lf_successor_only",
            ),
            _CaseSpec(
                case_name="cached_both_successor_components",
                description="Swap both preterminal successor-state handoff components z_on_next and z_lf_next back to the cached batch-start successors.",
                intervention_step_offsets=(-2, -1),
                successor_handoff_mode="cached_both_successor_components",
            ),
        )


@dataclass
class FMPCTF2SuccessorHandoffSourceSuiteRunResult:
    run_dir: Path
    config: dict[str, Any]
...
def _handoff_component_table() -> list[dict[str, Any]]:
    return [
        {
            "component_name": "z_on_next",
            "produced_by": "_plan_tf2_micro_step: z_on_k + dt * on_velocity",
            "default_source": "live_on_policy",
            "cached_source_available": True,
            "used_for": "successor_state_content; becomes next-step z_on_k and therefore controls next-step on-policy rollout geometry",
        },
        {
            "component_name": "z_lf_next",
            "produced_by": "_plan_tf2_micro_step: z_lf_k + dt * hidden_local_flow(context, z_lf_k)",
            "default_source": "live_local_field",
            "cached_source_available": True,
            "used_for": "successor_state_content; becomes next-step z_lf_k and therefore controls next-step detached local-field feature bundle / anchor generation",
        },
        {
            "component_name": "psi_inputs",
            "produced_by": "_plan_tf2_micro_step supervision bundle assembly",
            "default_source": "live_current_step",
            "cached_source_available": True,
            "used_for": "current-step direction anchor extraction and psi training inputs; not swapped in this successor-state-only diagnostic",
        },
        {
            "component_name": "boot_targets + identity_targets",
            "produced_by": "_plan_tf2_micro_step supervision target assembly",
            "default_source": "live_current_step",
            "cached_source_available": True,
            "used_for": "current-step regression targets only; not swapped in this successor-state-only diagnostic",
        },
    ]


def _suite_config_payload(config: FMPCTF2SuccessorHandoffSourceSuiteConfig) -> dict[str, Any]:
    return {
        "phase": "Phase Incremental Bridge",
        "stage": "adopted_package_successor_handoff_source_localization",
        ...
        "candidate_specs": [
            {
                ...
                "successor_handoff_mode": spec.successor_handoff_mode,
            }
            for spec in config.case_specs()
        ],
        "handoff_component_table": _handoff_component_table(),
...
def _success_row(
    ...
    *,
    ...
    return {
        ...
        "successor_handoff_mode": case_spec.successor_handoff_mode,
        "run_status": "success",
        "failure_kind": "",
...
def _gate_metrics_recovered(
    case_summary: dict[str, Any],
    config: FMPCTF2SuccessorHandoffSourceSuiteConfig,
) -> bool:
    return (
        float(case_summary["seed_gate_positive_rate"]) >= float(config.primary_gate_positive_rate_threshold)
        and float(case_summary["selected_epoch_passes_gate_rate"]) >= float(config.primary_selected_gate_rate_threshold)
        and float(case_summary["selector_fallback_used_rate"])
        <= float(config.primary_selector_fallback_used_rate_threshold)
    )
...
def _apply_successor_handoff_mode(
    mode: _SuccessorHandoffMode,
    *,
    live_z_on_next: np.ndarray,
    live_z_lf_next: np.ndarray,
    cached_slot: _ReplayPlanSlot,
) -> tuple[np.ndarray, np.ndarray]:
    if mode == "live_successor_handoff":
        return np.asarray(live_z_on_next, dtype=np.float64).copy(), np.asarray(live_z_lf_next, dtype=np.float64).copy()
    if mode == "cached_on_successor_only":
        return np.asarray(cached_slot.z_on_next, dtype=np.float64).copy(), np.asarray(live_z_lf_next, dtype=np.float64).copy()
    if mode == "cached_lf_successor_only":
        return np.asarray(live_z_on_next, dtype=np.float64).copy(), np.asarray(cached_slot.z_lf_next, dtype=np.float64).copy()
    if mode == "cached_both_successor_components":
        return np.asarray(cached_slot.z_on_next, dtype=np.float64).copy(), np.asarray(cached_slot.z_lf_next, dtype=np.float64).copy()
    raise ValueError(f"Unsupported successor_handoff_mode '{mode}'.")
...
def _run_customized_micro_step(
    ...
    *,
    ...
    is_custom_preterminal_step = (
        ...
        and case_spec.successor_handoff_mode != "live_successor_handoff"
    )
    ...
    if is_custom_preterminal_step:
        if cached_slot is None:
            raise ValueError("cached_slot is required for successor-handoff source localization.")
        z_on_next, z_lf_next = _apply_successor_handoff_mode(
            case_spec.successor_handoff_mode,
            live_z_on_next=z_on_next,
            live_z_lf_next=z_lf_next,
...
def _train_one_batch_successor_handoff_source(
    model: Any,
    psi_network: Any,
    config: FMPCTF2Config,
    x_batch: np.ndarray,
    ...
    *,
    ...
    cached_slots = (
        ...
        if case_spec.successor_handoff_mode != "live_successor_handoff"
        else None
    )
...
def _run_one_case_seed(
    suite_run_dir: Path,
    suite_config: FMPCTF2SuccessorHandoffSourceSuiteConfig,
    case_spec: _CaseSpec,
    seed: int,
) -> dict[str, Any]:
    set_seed(int(seed))
    ...
    for epoch_index in range(int(config.epochs)):
        ...
        for x_batch, y_batch in iter_minibatches(
            ...
            train_loss, boot_loss, identity_loss, transported_energy = _train_one_batch_successor_handoff_source(
                model,
                psi_network,
                config,
                x_batch,
    ...
    _write_run_artifacts(
        ...
        type(
            ...
            (),
            {
                ...
                "direction_source_mode": "detached_local_field",
                "norm_handling_mode": "keep_live_norm",
                "handoff_mode": case_spec.successor_handoff_mode,
            },
        )(),
        epoch_rows,
...
def _diagnose_and_recommend(
    ...
    cached_on = by_case["cached_on_successor_only"]
    cached_lf = by_case["cached_lf_successor_only"]
    cached_both = by_case["cached_both_successor_components"]

    on_gate = _gate_metrics_recovered(cached_on, config)
    lf_gate = _gate_metrics_recovered(cached_lf, config)
    both_gate = _gate_metrics_recovered(cached_both, config)

    evidence = {
        "failed_anchor_case": failed_anchor,
        "cached_on_case": cached_on,
        "cached_lf_case": cached_lf,
        "cached_both_case": cached_both,
        "recovery_by_case": recovery_by_case,
    }

    both_vs_on_accuracy = float(cached_both["mean_val_accuracy"]) - float(cached_on["mean_val_accuracy"])
    both_vs_on_rowspace = float(cached_on["mean_val_terminal_rowspace_rms"]) - float(
        cached_both["mean_val_terminal_rowspace_rms"]
    )

    if on_gate and not lf_gate:
        if both_gate and (
            both_vs_on_accuracy >= float(config.cross_source_accuracy_margin)
            or both_vs_on_rowspace >= float(config.cross_source_rowspace_margin)
        ):
            return (
                "cross_source_successor_inconsistency_is_primary_blocker",
                evidence,
                "run one minimal mixed-successor-handoff reformulation diagnostic that keeps the preterminal on-policy successor value but removes the remaining cross-source inconsistency at the successor-handoff boundary",
            )
        return (
            "stale_successor_value_is_primary_blocker",
            evidence,
            "run one confirmation-level reformulation on the preterminal on-policy successor-value component only, without reopening any cone-family sweep",
        )
    if lf_gate and not on_gate:
        return (
            "successor_feature_bundle_is_primary_blocker",
            evidence,
            "run one confirmation-level reformulation on the preterminal local-field successor bundle only, without reopening any cone-family sweep",
        )
    if both_gate and not on_gate and not lf_gate:
        return (
            "no_single_successor_component_dominates__interaction_blocker",
            evidence,
            "run one minimal mixed-successor-handoff reformulation diagnostic next, because no single successor component restores the gate contract by itself",
        )
    if on_gate and lf_gate:
        if both_vs_on_accuracy >= float(config.cross_source_accuracy_margin) or both_vs_on_rowspace >= float(
            config.cross_source_rowspace_margin
        ):
            return (
                "cross_source_successor_inconsistency_is_primary_blocker",
                evidence,
                "run one minimal mixed-successor-handoff reformulation diagnostic that directly resolves the remaining cross-source successor inconsistency",
            )
        return (
            "no_single_successor_component_dominates__interaction_blocker",
            evidence,
            "run one minimal mixed-successor-handoff reformulation diagnostic next, because the remaining blocker still looks interaction-dominated",
        )
    return (
        "no_single_successor_component_dominates__interaction_blocker",
        evidence,
        "run one minimal mixed-successor-handoff reformulation diagnostic next, because no single successor component dominates the remaining blocker",
    )


def run_fmpc_tf2_successor_handoff_source_suite(
    config: FMPCTF2SuccessorHandoffSourceSuiteConfig,
) -> FMPCTF2SuccessorHandoffSourceSuiteRunResult:
    ...
    return FMPCTF2SuccessorHandoffSourceSuiteRunResult(
        run_dir=run_dir,
