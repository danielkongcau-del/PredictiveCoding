...
@dataclass
class FMPCTF2SplitThresholdCouplingSuiteConfig:
    """Run a narrow adopted-package split-threshold terminal coupling diagnostic."""

    experiment_name: str = "fmpc_tf2_split_threshold_coupling_suite"
    ...
    def case_specs(self) -> tuple[_CaseSpec, ...]:
        return (
            ...
            _CaseSpec(
                case_name="split_threshold_row_strict",
                description="Keep both components active, with stricter row-space clip than orthogonal clip.",
            ...
            _CaseSpec(
                case_name="split_threshold_orthogonal_strict",
                description="Keep both components active, with stricter orthogonal clip than row-space clip.",
                terminal_intervention="local_field_direction_angle_clip_keep_live_norm_split_threshold",
...
@dataclass
class FMPCTF2SplitThresholdCouplingSuiteRunResult:
    run_dir: Path
    config: dict[str, Any]
...
def _suite_config_payload(config: FMPCTF2SplitThresholdCouplingSuiteConfig) -> dict[str, Any]:
    return {
        "phase": "Phase Incremental Bridge",
        "stage": "adopted_package_split_threshold_terminal_coupling",
...
def _case_run_id(case_name: str, seed: int) -> str:
    short_name = {
        ...
        "split_threshold_row_strict": "split_row",
        "split_threshold_orthogonal_strict": "split_orth",
...
def run_fmpc_tf2_split_threshold_coupling_suite(
    config: FMPCTF2SplitThresholdCouplingSuiteConfig,
    ...
    for case in config.case_specs():
        for seed in config.seeds:
            ...
            if result.model is None or result.psi_network is None:
                raise ValueError("Split-threshold coupling suite requires runtime model and psi network objects.")
    ...
    summary = {
        ...
        "should_promote_split_threshold_terminal_intervention": promoted_candidate_name is not None,
        "promoted_candidate_name": promoted_candidate_name,
        "decision": (
            "promote_split_threshold_terminal_intervention"
    ...
    return FMPCTF2SplitThresholdCouplingSuiteRunResult(
        run_dir=run_dir,
