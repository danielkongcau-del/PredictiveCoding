...
TerminalMode = Literal[
    ...
    "terminal_live_full_bundle",
    "terminal_replay_bootstrap_only",
    "terminal_replay_identity_only",
    "terminal_replay_full_supervision_bundle",
    "k4_open_loop_baseline_plan_replay",
]
TerminalBundleMode = Literal[
    "closed_loop_live_plan",
    "open_loop_baseline_plan_replay",
    "live_full_bundle",
    "replay_bootstrap_only",
    "replay_identity_only",
    "replay_full_supervision_bundle",
    "full_replay",
]
...
class _CellSpec:
    ...
    terminal_bundle_mode: TerminalBundleMode


@dataclass
class FMPCTF2TerminalStepSupervisionSplitSuiteConfig:
    """Diagnostic-only terminal-step supervision-bundle split suite."""

    experiment_name: str = "fmpc_tf2_terminal_step_supervision_split_suite"
...
@dataclass
class FMPCTF2TerminalStepSupervisionSplitSuiteRunResult:
    run_dir: Path
    config: dict[str, Any]
...
def _baseline_specs() -> list[_CellSpec]:
    return [
        _CellSpec(
            ...
            terminal_bundle_mode="closed_loop_live_plan",
        ),
        _CellSpec(
            candidate_key="baseline_plain_raw",
            mode="open_loop_baseline_plan_replay",
            replay_prefix_steps=4,
            terminal_bundle_mode="open_loop_baseline_plan_replay",
        ),
    ]
...
def _challenger_specs() -> list[_CellSpec]:
    return [
        _CellSpec(
            ...
            mode="terminal_live_full_bundle",
            replay_prefix_steps=3,
            terminal_bundle_mode="live_full_bundle",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
            mode="terminal_replay_bootstrap_only",
            replay_prefix_steps=3,
            terminal_bundle_mode="replay_bootstrap_only",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
            mode="terminal_replay_identity_only",
            replay_prefix_steps=3,
            terminal_bundle_mode="replay_identity_only",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
            mode="terminal_replay_full_supervision_bundle",
            replay_prefix_steps=3,
            terminal_bundle_mode="replay_full_supervision_bundle",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
            mode="k4_open_loop_baseline_plan_replay",
            replay_prefix_steps=4,
            terminal_bundle_mode="full_replay",
        ),
    ]
...
def _suite_config_payload(
    config: FMPCTF2TerminalStepSupervisionSplitSuiteConfig,
    candidates: dict[str, FMPCTF2PartialHandoffCandidate],
) -> dict[str, Any]:
    return {
        "phase": "Phase Incremental Bridge",
        "stage": "terminal_step_supervision_split_suite",
        ...
        "terminal_definition": (
            "All challenger modes replay cached baseline steps 0-2 and vary only the terminal "
            "step-3 supervision bundle applied to the challenger's own live terminal action."
        ),
    }
...
def _make_cell(
    ...
    *,
    seed: int,
    suite_config: FMPCTF2TerminalStepSupervisionSplitSuiteConfig,
) -> _CellRuntime:
...
def _terminal_bundle_metrics(
    ...
    if live_plan is None:
        return {
            ...
            "terminal_bootstrap_target_cosine_similarity": 1.0,
            "terminal_identity_target_cosine_similarity": 1.0,
            "terminal_lambda_id_delta": 0.0,
            "terminal_predicted_average_velocity_delta_vs_replay": 0.0,
            "terminal_state_slot_displacement": 0.0,
        }
    replay_inputs = _candidate_replay_inputs(config, baseline_slot)
    replay_predictions = fmpc_tf2_module._psi_predict(psi_network, replay_inputs, config)
    live_predictions = fmpc_tf2_module._psi_predict(psi_network, live_plan.psi_inputs, config)
    return {
        ...
        "terminal_bootstrap_target_cosine_similarity": _mean_cosine_similarity(
            baseline_slot.boot_targets,
            live_plan.boot_targets,
        ),
        "terminal_identity_target_cosine_similarity": _mean_cosine_similarity(
            baseline_slot.identity_targets,
            live_plan.identity_targets,
        ),
        "terminal_lambda_id_delta": abs(float(baseline_slot.lambda_id) - float(baseline_slot.lambda_id)),
        "terminal_predicted_average_velocity_delta_vs_replay": _relative_target_delta(
            replay_predictions,
            live_predictions,
        ),
        "terminal_state_slot_displacement": _mean_state_slot_displacement(
...
def _write_run_artifacts(
    ...
    *,
    ...
    config_payload["terminal_bundle_mode"] = spec.terminal_bundle_mode
...
def _train_one_batch_terminal_split(
    model: Any,
    psi_network: Any,
    config: FMPCTF2Config,
    x_batch: np.ndarray,
    ...
    *,
    ...
    if config.supervision_policy != "local_only":
        raise ValueError("Terminal supervision split diagnostic is defined only for local_only supervision.")
    if fmpc_tf2_module._resolved_theta_update_cadence(config) != "terminal_only":
        raise ValueError("Terminal supervision split diagnostic expects terminal_only theta cadence.")
    ...
    for step_index, baseline_slot in enumerate(baseline_slots):
        use_replay_step = step_index < int(spec.replay_prefix_steps)
        if use_replay_step:
            ...
            live_boot_targets = live_plan.boot_targets
            live_identity_targets = live_plan.identity_targets
            live_lambda_id = float(baseline_slot.lambda_id)
            z_on_next = live_plan.z_on_next.copy()
            z_lf_next = live_plan.z_lf_next.copy()

            if spec.terminal_bundle_mode == "live_full_bundle":
                boot_targets = live_boot_targets
                identity_targets = live_identity_targets
                lambda_id = live_lambda_id
            elif spec.terminal_bundle_mode == "replay_bootstrap_only":
                boot_targets = baseline_slot.boot_targets
                identity_targets = live_identity_targets
                lambda_id = live_lambda_id
            elif spec.terminal_bundle_mode == "replay_identity_only":
                boot_targets = live_boot_targets
                identity_targets = baseline_slot.identity_targets
                lambda_id = float(baseline_slot.lambda_id)
            elif spec.terminal_bundle_mode == "replay_full_supervision_bundle":
                boot_targets = baseline_slot.boot_targets
                identity_targets = baseline_slot.identity_targets
                lambda_id = float(baseline_slot.lambda_id)
            elif spec.terminal_bundle_mode == "full_replay":
                raise ValueError("full_replay must not reach the live terminal branch.")
            elif spec.terminal_bundle_mode == "closed_loop_live_plan":
                boot_targets = live_boot_targets
                identity_targets = live_identity_targets
                lambda_id = live_lambda_id
            else:
                raise ValueError(f"Unsupported terminal_bundle_mode '{spec.terminal_bundle_mode}'.")
            terminal_metrics = _terminal_bundle_metrics(config, psi_network, baseline_slot, live_plan)
    ...
    if terminal_metrics is None:
        terminal_metrics = _terminal_bundle_metrics(config, psi_network, baseline_slots[-1], None)
...
def _finalize_cell(
    ...
    *,
    ...
    summary = {
        "phase": "Phase Incremental Bridge",
        "stage": "terminal_step_supervision_split_diagnostic",
        "diagnostic_only": True,
        ...
        "terminal_bundle_mode": cell.spec.terminal_bundle_mode,
        "teacher_free": True,
...
def _success_run_row(*, artifact: _RunArtifacts, base_run_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "terminal_bundle_mode": artifact.summary["terminal_bundle_mode"],
...
def _aggregate_terminal_drift(terminal_rows: list[dict[str, Any]], *, mode: TerminalMode) -> dict[str, float | None]:
    ...
    if not relevant:
        return {
            ...
            "mean_terminal_bootstrap_target_cosine_similarity": None,
            "mean_terminal_identity_target_cosine_similarity": None,
            "mean_terminal_lambda_id_delta": None,
            "mean_terminal_predicted_average_velocity_delta_vs_replay": None,
    ...
    return {
        ...
        "mean_terminal_bootstrap_target_cosine_similarity": _mean(
            [float(row["terminal_bootstrap_target_cosine_similarity"]) for row in relevant]
        ),
        "mean_terminal_identity_target_cosine_similarity": _mean(
            [float(row["terminal_identity_target_cosine_similarity"]) for row in relevant]
        ),
        "mean_terminal_lambda_id_delta": _mean([float(row["terminal_lambda_id_delta"]) for row in relevant]),
        "mean_terminal_predicted_average_velocity_delta_vs_replay": _mean(
            [float(row["terminal_predicted_average_velocity_delta_vs_replay"]) for row in relevant]
        ),
...
def _run_one_seed(
    ...
    *,
    ...
    cells: dict[tuple[str, TerminalMode], _CellRuntime] = {}
    ...
    for epoch_index in range(int(baseline_control.config.epochs)):
        ...
        epoch_metric_lists: dict[tuple[str, TerminalMode], dict[str, list[float]]] = {
            cell_key: {
                "train_loss": [],
                "train_boot_loss": [],
                "train_identity_loss": [],
        ...
        epoch_terminal_metric_lists: dict[TerminalMode, dict[str, list[float]]] = {
            spec.mode: {
                "relative_terminal_bootstrap_target_delta": [],
                "relative_terminal_identity_target_delta": [],
                "terminal_bootstrap_target_cosine_similarity": [],
                "terminal_identity_target_cosine_similarity": [],
                "terminal_lambda_id_delta": [],
                "terminal_predicted_average_velocity_delta_vs_replay": [],
        ...
        for x_batch, y_batch in fmpc_tf2_module.iter_minibatches(
            ...
            for spec in _cell_specs():
                ...
                train_loss, boot_loss, identity_loss, transported_energy, terminal_metrics = _train_one_batch_terminal_split(
                    cell.model,
                    cell.psi_network,
                    cell.config,
                    x_batch,
            ...
            terminal_drift_rows.append(
                {
                    ...
                    "terminal_bundle_mode": spec.terminal_bundle_mode,
                    **aggregated,
                }
            )
...
def run_fmpc_tf2_terminal_step_supervision_split_suite(
    config: FMPCTF2TerminalStepSupervisionSplitSuiteConfig,
) -> FMPCTF2TerminalStepSupervisionSplitSuiteRunResult:
    ...
    _write_csv(run_dir / "terminal_bundle_drift_epoch_metrics.csv", terminal_drift_rows)
    ...
    challenger_modes: list[TerminalMode] = [
        "terminal_live_full_bundle",
        "terminal_replay_bootstrap_only",
        "terminal_replay_identity_only",
        "terminal_replay_full_supervision_bundle",
    ...
    def _rescues(mode: TerminalMode) -> bool:
        same_family = pairwise_vs_terminal_live[mode]
    ...
    bootstrap_rescue = _rescues("terminal_replay_bootstrap_only")
    identity_rescue = _rescues("terminal_replay_identity_only")
    full_bundle_rescue = _rescues("terminal_replay_full_supervision_bundle")

    if bootstrap_rescue and not identity_rescue:
        dominant_interpretation = "terminal_bootstrap_target_mismatch_is_dominant"
        next_single_narrow_move = (
            "run one narrow terminal bootstrap sub-bundle diagnostic that separates target value replay "
            "from terminal bootstrap-input/state conditioning without reopening broader TF2 search"
        )
    elif identity_rescue and not bootstrap_rescue:
        dominant_interpretation = "terminal_identity_side_coupling_is_dominant"
        next_single_narrow_move = (
            "run one narrow terminal identity-side decomposition diagnostic that separates identity target "
            "replay from JVP-side provenance under the same benign 0-2 replay scaffold"
        )
    elif full_bundle_rescue and not bootstrap_rescue and not identity_rescue:
        dominant_interpretation = "terminal_bootstrap_and_identity_bundle_interaction_is_dominant"
        next_single_narrow_move = (
            "run one narrow terminal bundle interaction diagnostic that replays the bundle but perturbs only "
            "one bundle component at a time around the rescued full-bundle setting"
        )
    elif not bootstrap_rescue and not identity_rescue and not full_bundle_rescue:
        dominant_interpretation = "terminal_live_action_prediction_is_more_problematic_than_supervision_bundle"
        next_single_narrow_move = (
            "run one narrow terminal-step action-output stabilization diagnostic that keeps supervision replayed "
            "but targets the challenger's own terminal transition/prediction rather than target construction"
        )
    else:
        dominant_interpretation = "terminal_failure_shows_mixed_supervision_and_action_coupling"
        next_single_narrow_move = (
            "run one narrow terminal action-versus-bundle interaction diagnostic around the partially rescued "
            "mode instead of reopening any broader TF2 mechanism search"
        )
    ...
    summary = {
        ...
        "terminal_bundle_drift_summary": terminal_bundle_drift_summary,
        "pairwise_delta_vs_baseline_closed_loop_run": pairwise_vs_baseline_live,
        "pairwise_delta_vs_challenger_terminal_live_full_bundle": pairwise_vs_terminal_live,
        "pairwise_delta_vs_challenger_k4_open_loop_replay_run": pairwise_vs_k4,
        "is_terminal_bootstrap_target_mismatch_dominant": bool(bootstrap_rescue and not identity_rescue),
        "is_terminal_identity_side_coupling_dominant": bool(identity_rescue and not bootstrap_rescue),
        "is_terminal_supervision_bundle_interaction_dominant": bool(
            full_bundle_rescue and not bootstrap_rescue and not identity_rescue
        ),
        "is_terminal_live_action_prediction_dominant": bool(
            not bootstrap_rescue and not identity_rescue and not full_bundle_rescue
        ),
        ...
        "artifacts": {
            ...
            "terminal_bundle_drift_epoch_metrics_csv": "terminal_bundle_drift_epoch_metrics.csv",
        },
    }
    _write_json(run_dir / "aggregate_summary.json", summary)
    return FMPCTF2TerminalStepSupervisionSplitSuiteRunResult(
        run_dir=run_dir,
