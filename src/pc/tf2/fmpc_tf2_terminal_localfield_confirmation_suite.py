...
TerminalLocalFieldConfirmationMode = Literal[
...
class _CellSpec:
    candidate_key: str
    stabilizer_mode: TerminalLocalFieldConfirmationMode
...
@dataclass
class FMPCTF2TerminalLocalFieldConfirmationSuiteConfig:
    """Package-level terminal local-field confirmation suite."""

    experiment_name: str = "fmpc_tf2_terminal_localfield_confirmation_suite"
    ...
    confirmation_test_gain_vs_baseline: float = 0.005
...
@dataclass
class FMPCTF2TerminalLocalFieldConfirmationSuiteRunResult:
    run_dir: Path
    config: dict[str, Any]
...
def _suite_config_payload(
    config: FMPCTF2TerminalLocalFieldConfirmationSuiteConfig,
    candidates: dict[str, FMPCTF2PartialHandoffCandidate],
) -> dict[str, Any]:
    return {
        "phase": "Phase TF2",
        "stage": "terminal_localfield_confirmation_suite",
        "confirmation_oriented": True,
        ...
        "comparison_cells": [
            "baseline_plain_raw__baseline_closed_loop_live_plan",
            "residualized_local_field_poly_rt2__terminal_live_raw_closed_loop",
            "residualized_local_field_poly_rt2__terminal_local_field_direction_angle_clip_keep_live_norm_deg30",
            "residualized_local_field_poly_rt2__terminal_local_field_direction_hard_replace_keep_live_norm",
        ],
        "fixed": {
        ...
        "confirmation_thresholds": {
            "mean_test_accuracy_gain_vs_baseline_closed_loop": float(config.confirmation_test_gain_vs_baseline),
...
def _make_cell(
    ...
    *,
    seed: int,
    suite_config: FMPCTF2TerminalLocalFieldConfirmationSuiteConfig,
) -> _CellRuntime:
...
def _train_one_batch_terminal_localfield_confirmation(
    model: Any,
    psi_network: Any,
    config: FMPCTF2Config,
    x_batch: np.ndarray,
...
def _write_run_artifacts(
    ...
    *,
    ...
    config_payload["confirmation_oriented"] = True
...
def _finalize_cell(
    ...
    *,
    ...
    summary = {
        "phase": "Phase TF2",
        "stage": "terminal_localfield_confirmation",
...
def _pairwise_vs_reference(
    ...
    *,
    ...
    reference_stabilizer_mode: TerminalLocalFieldConfirmationMode,
) -> dict[str, Any]:
    candidate_by_seed = {
        int(row["seed"]): row
        for row in run_rows
...
def _run_one_seed(
    ...
    *,
    ...
    cells: dict[tuple[str, TerminalLocalFieldConfirmationMode], _CellRuntime] = {}
    ...
    for epoch_index in range(int(baseline_control.config.epochs)):
        ...
        epoch_metric_lists: dict[tuple[str, TerminalLocalFieldConfirmationMode], dict[str, list[float]]] = {
            cell_key: {
                "train_loss": [],
                "train_boot_loss": [],
                "train_identity_loss": [],
        ...
        for x_batch, y_batch in fmpc_tf2_module.iter_minibatches(
            ...
            for spec in _cell_specs():
                ...
                train_loss, boot_loss, identity_loss, transported_energy = (
                    _train_one_batch_terminal_localfield_confirmation(
                        cell.model,
                        cell.psi_network,
                        cell.config,
                        x_batch,
...
def run_fmpc_tf2_terminal_localfield_confirmation_suite(
    config: FMPCTF2TerminalLocalFieldConfirmationSuiteConfig,
) -> FMPCTF2TerminalLocalFieldConfirmationSuiteRunResult:
    ...
    challenger_modes: list[TerminalLocalFieldConfirmationMode] = [
        "terminal_live_raw_closed_loop",
        "terminal_local_field_direction_hard_replace_keep_live_norm",
        "terminal_local_field_direction_angle_clip_keep_live_norm_deg30",
    ]
    ...
    angle_mode = "terminal_local_field_direction_angle_clip_keep_live_norm_deg30"
    hard_mode = "terminal_local_field_direction_hard_replace_keep_live_norm"
    raw_mode = "terminal_live_raw_closed_loop"

    angle_vs_baseline = _pairwise_vs_reference(
        run_rows,
        candidate_key="residualized_local_field_poly_rt2",
        stabilizer_mode=angle_mode,
        reference_candidate_key="baseline_plain_raw",
        reference_stabilizer_mode="baseline_closed_loop_live_plan",
    )
    angle_vs_raw = _pairwise_vs_reference(
        run_rows,
        candidate_key="residualized_local_field_poly_rt2",
        stabilizer_mode=angle_mode,
        reference_candidate_key="residualized_local_field_poly_rt2",
        reference_stabilizer_mode=raw_mode,
    )
    hard_vs_angle = _pairwise_vs_reference(
        run_rows,
        candidate_key="residualized_local_field_poly_rt2",
        stabilizer_mode=hard_mode,
        reference_candidate_key="residualized_local_field_poly_rt2",
        reference_stabilizer_mode=angle_mode,
    )

    angle_summary = challenger_summary[angle_mode]
    hard_summary = challenger_summary[hard_mode]
    angle_beats_baseline = bool(
        float(angle_vs_baseline["mean_test_accuracy_delta"]) >= float(config.confirmation_test_gain_vs_baseline)
        and float(angle_vs_baseline["mean_val_accuracy_delta"]) >= 0.0
    )
    angle_gate_robust = bool(
        float(angle_summary["seed_gate_positive_rate"]) > float(config.required_gate_seed_majority_rate)
        and float(angle_summary["selected_epoch_passes_gate_rate"]) >= float(config.selected_epoch_passes_gate_rate_floor)
    )
    angle_runtime_modest = bool(
        float(angle_vs_raw["mean_runtime_delta"]) <= float(config.modest_runtime_overhead_seconds)
    )
    hard_has_better_test = float(hard_vs_angle["mean_test_accuracy_delta"]) > 0.0
    hard_has_worse_gate = bool(
        float(hard_summary["seed_gate_positive_rate"]) < float(angle_summary["seed_gate_positive_rate"])
        or float(hard_summary["selected_epoch_passes_gate_rate"]) < float(angle_summary["selected_epoch_passes_gate_rate"])
        or float(hard_summary["selector_fallback_used_rate"]) > float(angle_summary["selector_fallback_used_rate"])
    )
    hard_replace_only_upper_bound = bool(hard_has_worse_gate)
    should_adopt_angle_clip_package = bool(
        angle_beats_baseline and angle_gate_robust and angle_runtime_modest and hard_replace_only_upper_bound
    )

    if should_adopt_angle_clip_package:
        recommended_package = "residualized_local_field_poly_rt2 + terminal_local_field_direction_angle_clip_keep_live_norm_deg30"
        dominant_interpretation = "angle_clip_package_is_strong_enough_to_replace_the_plain_tf2_experimental_default"
        recommendation_reason = (
            "The angle-clip package materially beats the plain baseline, remains gate-robust under the canonical selector, "
            "and offers a better robustness tradeoff than hard replace."
        )
        next_single_narrow_move = (
            "run one narrow package-adoption patch that promotes the clip30 terminal local-field stabilizer into the next TF2 experimental default"
        )
    else:
        recommended_package = None
        dominant_interpretation = "angle_clip_package_is_not_yet_confirmed_as_a_package_level_default_replacement"
        recommendation_reason = (
            "The package-level confirmation did not clear all adoption conditions against the current plain baseline and the hard-replace control."
        )
        next_single_narrow_move = (
            "run one narrow follow-up on the late-step package tradeoff rather than replacing the current plain TF2 experimental default"
        )

    tradeoff_note = (
        "Hard replace remains an upper-bound control if it preserves stronger mean test but loses on gate robustness or selector stability."
    )
    ...
    summary = {
        ...
        "pairwise_delta_angle_clip_package_vs_baseline_closed_loop_run": angle_vs_baseline,
        "pairwise_delta_angle_clip_package_vs_terminal_live_raw_closed_loop": angle_vs_raw,
        "pairwise_delta_hard_replace_vs_angle_clip_package": hard_vs_angle,
        "confirmation_thresholds": {
            "mean_test_accuracy_gain_vs_baseline_closed_loop": float(config.confirmation_test_gain_vs_baseline),
            "required_gate_seed_majority_rate": float(config.required_gate_seed_majority_rate),
            "selected_epoch_passes_gate_rate_floor": float(config.selected_epoch_passes_gate_rate_floor),
            "modest_runtime_overhead_seconds": float(config.modest_runtime_overhead_seconds),
            "mean_val_accuracy_not_worse_than_baseline_closed_loop": True,
        },
        "does_angle_clip_package_beat_plain_baseline": bool(angle_beats_baseline),
        "is_angle_clip_package_gain_robust_across_seeds_and_gate_constrained_selection": bool(angle_gate_robust),
        "is_angle_clip_package_runtime_overhead_modest": bool(angle_runtime_modest),
        "is_hard_replace_still_only_upper_bound_control": bool(hard_replace_only_upper_bound),
        "hard_replace_has_higher_mean_test_than_angle_clip": bool(hard_has_better_test),
        "should_adopt_angle_clip_package_as_next_tf2_experimental_default": bool(should_adopt_angle_clip_package),
        "recommended_next_tf2_experimental_default_package": recommended_package,
    ...
    return FMPCTF2TerminalLocalFieldConfirmationSuiteRunResult(
        run_dir=run_dir,
