...
from ..tf1.fmpc_tf1_flow import build_tf1_context
...
@dataclass
class FMPCTF2UnifiedConeShapeSuiteConfig:
    """Run a narrow adopted-package unified-cone-shape diagnostic."""

    experiment_name: str = "fmpc_tf2_unified_cone_shape_suite"
    ...
    reference_summary_path: str | Path = "outputs/tf2/fmpc_tf2_external_comparison_suite/aggregate_summary.json"
    ...
    def case_specs(self) -> tuple[_CaseSpec, ...]:
        return (
            ...
            _CaseSpec(
                ...
                description="Same full-space cone family, but clip to a stricter interior target inside the current 30 degree cone.",
                terminal_intervention="local_field_direction_angle_clip_keep_live_norm",
                full_angle_clip_degrees=20.0,
                rowspace_angle_clip_degrees=20.0,
                orthogonal_angle_clip_degrees=20.0,
            ),
            _CaseSpec(
                case_name="unified_cone_relaxed_boundary_40",
                description="Same full-space cone family, but allow a slightly looser terminal boundary than the current 30 degree clip.",
                terminal_intervention="local_field_direction_angle_clip_keep_live_norm",
                full_angle_clip_degrees=40.0,
                rowspace_angle_clip_degrees=40.0,
                orthogonal_angle_clip_degrees=40.0,
            ),
        )


@dataclass
class FMPCTF2UnifiedConeShapeSuiteRunResult:
    run_dir: Path
    config: dict[str, Any]
...
def _suite_config_payload(config: FMPCTF2UnifiedConeShapeSuiteConfig) -> dict[str, Any]:
    return {
        "phase": "Phase TF2",
        "stage": "adopted_package_unified_cone_shape_diagnostic",
        ...
        "diagnostics": {
            ...
            "unified_full_space_cone_shape_comparison": True,
...
def _case_run_id(case_name: str, seed: int) -> str:
    short_name = {
        ...
        "unified_cone_interior_margin_20": "interior20",
        "unified_cone_relaxed_boundary_40": "relaxed40",
    }[case_name]
...
def _diagnose(summary_by_candidate: dict[str, dict[str, Any]]) -> tuple[str, dict[str, float], str]:
    ...
    interior = summary_by_candidate["unified_cone_interior_margin_20"]
    relaxed = summary_by_candidate["unified_cone_relaxed_boundary_40"]

    evidence = {
        "control_mean_val_accuracy": float(control["mean_val_accuracy"]),
        "interior_margin_mean_val_accuracy": float(interior["mean_val_accuracy"]),
        "relaxed_boundary_mean_val_accuracy": float(relaxed["mean_val_accuracy"]),
        "control_mean_test_accuracy": float(control["mean_test_accuracy"]),
        "interior_margin_mean_test_accuracy": float(interior["mean_test_accuracy"]),
        "relaxed_boundary_mean_test_accuracy": float(relaxed["mean_test_accuracy"]),
        "control_mean_gate_passing_epoch_count": float(control["mean_gate_passing_epoch_count"]),
        "interior_margin_mean_gate_passing_epoch_count": float(interior["mean_gate_passing_epoch_count"]),
        "relaxed_boundary_mean_gate_passing_epoch_count": float(relaxed["mean_gate_passing_epoch_count"]),
        "control_selected_epoch_passes_gate_rate": float(control["selected_epoch_passes_gate_rate"]),
        "interior_margin_selected_epoch_passes_gate_rate": float(interior["selected_epoch_passes_gate_rate"]),
        "relaxed_boundary_selected_epoch_passes_gate_rate": float(relaxed["selected_epoch_passes_gate_rate"]),
        "control_selector_fallback_used_rate": float(control["selector_fallback_used_rate"]),
        "interior_margin_selector_fallback_used_rate": float(interior["selector_fallback_used_rate"]),
        "relaxed_boundary_selector_fallback_used_rate": float(relaxed["selector_fallback_used_rate"]),
        "control_mean_val_full_space_angle_stabilized_vs_lf_deg": float(
            control["mean_val_full_space_angle_stabilized_vs_lf_deg"]
        ),
        "interior_margin_mean_val_full_space_angle_stabilized_vs_lf_deg": float(
            interior["mean_val_full_space_angle_stabilized_vs_lf_deg"]
        ),
        "relaxed_boundary_mean_val_full_space_angle_stabilized_vs_lf_deg": float(
            relaxed["mean_val_full_space_angle_stabilized_vs_lf_deg"]
        ),
        ...
        "interior_margin_mean_val_full_space_angle_above_30deg_rate_after_stabilization": float(
            interior["mean_val_full_space_angle_above_30deg_rate_after_stabilization"]
        ),
        "relaxed_boundary_mean_val_full_space_angle_above_30deg_rate_after_stabilization": float(
            relaxed["mean_val_full_space_angle_above_30deg_rate_after_stabilization"]
        ),
        "control_mean_val_delta_h_rowspace_fraction": float(control["mean_val_delta_h_rowspace_fraction"]),
        "interior_margin_mean_val_delta_h_rowspace_fraction": float(interior["mean_val_delta_h_rowspace_fraction"]),
        "relaxed_boundary_mean_val_delta_h_rowspace_fraction": float(relaxed["mean_val_delta_h_rowspace_fraction"]),
    }

    diagnosis = "current_hard_30_degree_unified_cone_remains_best_supported_after_accuracy_vs_robustness_tradeoff"
    if (
        float(interior["mean_val_accuracy"]) > float(control["mean_val_accuracy"])
        and float(interior["mean_test_accuracy"]) > float(control["mean_test_accuracy"])
        and float(interior["mean_gate_passing_epoch_count"]) < float(control["mean_gate_passing_epoch_count"])
    ):
        diagnosis = (
            "the_20_degree_interior_margin_variant_improves_accuracy_and_rowspace_metrics_but_loses_gate_robustness"
        )

    recommended_next_move = (
        "if TF2 work continues inside the adopted package, test at most one smooth unified-cone projection variant "
        "that tries to keep the interior-margin gain without paying the current gate-robustness cost"
    )
...
def _qualifies_for_adoption(
    ...
    return (
        ...
        and float(pairwise_vs_control["mean_val_full_space_angle_above_30deg_rate_after_stabilization_delta"]) <= 0.0
        ...
        and float(pairwise_vs_control["mean_selected_epoch_passes_gate_rate_delta"])
        >= -float(config.max_selected_gate_rate_drop)
        and float(pairwise_vs_control["mean_selector_fallback_used_rate_delta"])
        <= float(config.max_selector_fallback_increase)
    )


def run_fmpc_tf2_unified_cone_shape_suite(
    config: FMPCTF2UnifiedConeShapeSuiteConfig,
    ...
    promoted_candidate_name = promotion_candidates[0] if promotion_candidates else None
    ...
    summary = {
        ...
        "should_promote_refined_unified_cone_shape": promoted_candidate_name is not None,
        "promoted_candidate_name": promoted_candidate_name,
        "decision": (
            "promote_refined_unified_cone_shape"
    ...
    return FMPCTF2UnifiedConeShapeSuiteRunResult(
        run_dir=run_dir,
