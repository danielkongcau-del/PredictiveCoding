...
_Diagnosis = Literal[
    "increment_only_reformulation_is_adoption_viable",
    "increment_only_reformulation_recovers_partially_but_not_adoption_level",
...
@dataclass
class FMPCTF2SuccessorIncrementConfirmationSuiteConfig:
    """Run a very narrow confirmation-level reformulation on the preterminal successor increment."""

    experiment_name: str = "fmpc_tf2_successor_increment_confirmation_suite"
    ...
    def case_specs(self) -> tuple[_CaseSpec, ...]:
        return (
            ...
            _CaseSpec(
                ...
                description="Increment-only direction trust-region toward the cached increment, keeping the live increment norm unchanged.",
                intervention_step_offsets=(-2, -1),
...
@dataclass
class FMPCTF2SuccessorIncrementConfirmationSuiteRunResult:
    run_dir: Path
    config: dict[str, Any]
    ...
    return [
        {
            ...
            "used_for": "predecessor carry state for successor increment composition",
        },
        {
            "component_name": "z_on_next_live",
            "where_produced": "_plan_tf2_micro_step: z_on_k + dt * on_velocity, then optionally stabilized by the existing full-vector terminal-angle clip on the preterminal step",
            "source_type": "live_on_policy",
            "used_for": "live successor value before increment-only reformulation",
        },
        {
            "component_name": "delta_on_live",
            "where_produced": "derived inside this suite as z_on_next_live - z_on_k_live",
            "source_type": "live_on_policy",
            "used_for": "preterminal live successor increment under test",
        },
        {
            ...
            "used_for": "cached analogue used either as lower-bound replacement or as the direction anchor for the trust-region reformulation",
        },
    ]


def _suite_config_payload(config: FMPCTF2SuccessorIncrementConfirmationSuiteConfig) -> dict[str, Any]:
    return {
        "phase": "FMPC Stage 04 Incremental Bridge",
        "stage": "adopted_package_preterminal_successor_increment_confirmation",
...
def _control_relative_recovery(control: dict[str, Any], candidate: dict[str, Any]) -> dict[str, Any]:
...
def _gate_contract_intact(
    config: FMPCTF2SuccessorIncrementConfirmationSuiteConfig,
...
def _increment_direction_trust_region_keep_live_norm(
    ...
    *,
    ...
    stabilized = stabilized_direction * live_norm
    ensure_finite_array(stabilized, "tf2_increment_direction_trust_region")
    return stabilized
...
def _run_customized_micro_step(
    ...
    *,
    ...
    case_spec: _CaseSpec,
    suite_config: FMPCTF2SuccessorIncrementConfirmationSuiteConfig,
    ...
    if is_custom_preterminal_step:
        if cached_slot is None:
            raise ValueError("cached_slot is required for successor-increment confirmation.")
...
def _train_one_batch_successor_increment_confirmation(
    model: Any,
    psi_network: Any,
    config: FMPCTF2Config,
    x_batch: np.ndarray,
...
def _run_one_case_seed(
    ...
    for epoch_index in range(int(config.epochs)):
        ...
        for x_batch, y_batch in iter_minibatches(
            ...
            train_loss, boot_loss, identity_loss, transported_energy = _train_one_batch_successor_increment_confirmation(
                model,
                psi_network,
                config,
                x_batch,
...
def _diagnose_and_recommend(
    ...
    candidate = by_case["increment_direction_trust_region_keep_live_norm_30"]
    candidate_recovery = recovery_vs_failed_anchor["increment_direction_trust_region_keep_live_norm_30"]
    lower_recovery = recovery_vs_failed_anchor["live_carry_cached_increment_lower_bound"]
    gate_contract_intact = _gate_contract_intact(config, candidate)
    control_competitive = (
        float(candidate["mean_val_accuracy"]) + float(config.viable_max_val_accuracy_regression_vs_control)
        >= float(control["mean_val_accuracy"])
        and float(candidate["mean_test_accuracy"]) + float(config.viable_max_test_accuracy_regression_vs_control)
        >= float(control["mean_test_accuracy"])
    )
    materially_beats_lower_bound = (
        float(candidate_recovery["accuracy_gain_retained_fraction_from_control_to_failed_anchor"])
        > float(lower_recovery["accuracy_gain_retained_fraction_from_control_to_failed_anchor"])
        and float(candidate_recovery["terminal_rowspace_rms_gain_retained_fraction_from_control_to_failed_anchor"])
        > float(lower_recovery["terminal_rowspace_rms_gain_retained_fraction_from_control_to_failed_anchor"])
    )
    evidence = {
        ...
        "candidate_recovery_vs_failed_anchor": candidate_recovery,
        "candidate_recovery_vs_control": control_relative_recovery["increment_direction_trust_region_keep_live_norm_30"],
        "safe_lower_bound_recovery_vs_failed_anchor": lower_recovery,
        "safe_lower_bound_recovery_vs_control": control_relative_recovery["live_carry_cached_increment_lower_bound"],
    }
    if (
        gate_contract_intact
        and control_competitive
        and materially_beats_lower_bound
        and float(candidate_recovery["accuracy_gain_retained_fraction_from_control_to_failed_anchor"])
        ...
        return (
            "increment_only_reformulation_is_adoption_viable",
            evidence,
            "run one minimal adopted-package follow-up to confirm defaultization of the increment-only reformulation against the current adopted control",
        )
    if gate_contract_intact and materially_beats_lower_bound:
        return (
            "increment_only_reformulation_recovers_partially_but_not_adoption_level",
            evidence,
            "run one narrower increment-internal source-localization step next, keeping the work on the preterminal successor increment only",
        )
    return (
        "live_successor_increment_blocker_persists",
        evidence,
        "run the next narrower increment-internal source-localization step rather than reopening any cone-family or broader successor-value sweep",
    )


def run_fmpc_tf2_successor_increment_confirmation_suite(
    config: FMPCTF2SuccessorIncrementConfirmationSuiteConfig,
) -> FMPCTF2SuccessorIncrementConfirmationSuiteRunResult:
    ...
    return FMPCTF2SuccessorIncrementConfirmationSuiteRunResult(
        run_dir=run_dir,
