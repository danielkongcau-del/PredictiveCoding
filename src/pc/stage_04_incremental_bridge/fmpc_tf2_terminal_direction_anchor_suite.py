...
TerminalDirectionAnchorMode = Literal[
    "closed_loop_live_plan",
    "open_loop_baseline_plan_replay",
    "terminal_live_action_raw",
    "terminal_baseline_direction_only_keep_live_norm",
    "terminal_baseline_direction_mix_half_keep_live_norm",
    "terminal_local_field_direction_only_keep_live_norm",
    "terminal_local_field_direction_mix_half_keep_live_norm",
    "terminal_replay_action_vector_only",
    "k4_open_loop_baseline_plan_replay",
]
DirectionAnchorInterventionMode = Literal[
    "closed_loop_live_plan",
    "open_loop_baseline_plan_replay",
    "live_action_raw",
    "baseline_direction_only_keep_live_norm",
    "baseline_direction_mix_half_keep_live_norm",
    "local_field_direction_only_keep_live_norm",
    "local_field_direction_mix_half_keep_live_norm",
...
class _CellSpec:
    candidate_key: str
    mode: TerminalDirectionAnchorMode
    replay_prefix_steps: int
    direction_anchor_mode: DirectionAnchorInterventionMode


@dataclass
class FMPCTF2TerminalDirectionAnchorSuiteConfig:
    """Diagnostic-only terminal-step direction-anchor suite."""

    experiment_name: str = "fmpc_tf2_terminal_direction_anchor_suite"
...
@dataclass
class FMPCTF2TerminalDirectionAnchorSuiteRunResult:
    run_dir: Path
    config: dict[str, Any]
    run_rows: list[dict[str, Any]]
    direction_drift_rows: list[dict[str, Any]]
...
def _challenger_specs() -> list[_CellSpec]:
    return [
        _CellSpec(
            ...
            direction_anchor_mode="live_action_raw",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
            mode="terminal_baseline_direction_only_keep_live_norm",
            replay_prefix_steps=3,
            direction_anchor_mode="baseline_direction_only_keep_live_norm",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
            mode="terminal_baseline_direction_mix_half_keep_live_norm",
            replay_prefix_steps=3,
            direction_anchor_mode="baseline_direction_mix_half_keep_live_norm",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
            mode="terminal_local_field_direction_only_keep_live_norm",
            replay_prefix_steps=3,
            direction_anchor_mode="local_field_direction_only_keep_live_norm",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
            mode="terminal_local_field_direction_mix_half_keep_live_norm",
            replay_prefix_steps=3,
            direction_anchor_mode="local_field_direction_mix_half_keep_live_norm",
        ),
        _CellSpec(
            candidate_key="residualized_local_field_poly_rt2",
        ...
        _CellSpec(
            ...
            replay_prefix_steps=4,
            direction_anchor_mode="full_replay",
        ),
    ]
...
def _suite_config_payload(
    ...
    return {
        ...
        "terminal_definition": (
            ...
            "direction anchor under the challenger's own live action norm."
        ),
    }
...
def _make_cell(
    ...
    *,
    seed: int,
    suite_config: FMPCTF2TerminalDirectionAnchorSuiteConfig,
) -> _CellRuntime:
...
def _direction_anchor_metrics(
    *,
    ...
    return {
        ...
        "terminal_post_action_state_slot_displacement": _mean_state_displacement(
            live_z_on_next,
            mode_z_on_next,
        ),
...
def _train_one_batch_terminal_direction_anchor(
    model: Any,
    psi_network: Any,
    config: FMPCTF2Config,
    x_batch: np.ndarray,
    ...
    *,
    ...
    if config.supervision_policy != "local_only":
        raise ValueError("Terminal direction-anchor diagnostic is defined only for local_only supervision.")
    if fmpc_tf2_module._resolved_theta_update_cadence(config) != "terminal_only":
        raise ValueError("Terminal direction-anchor diagnostic expects terminal_only theta cadence.")
    ...
    for step_index, baseline_slot in enumerate(baseline_slots):
        if spec.direction_anchor_mode == "closed_loop_live_plan":
        ...
        elif step_index < int(spec.replay_prefix_steps) or spec.direction_anchor_mode in {
            "open_loop_baseline_plan_replay",
            "full_replay",
        }:
            ...
            if spec.direction_anchor_mode == "live_action_raw":
                mode_action = live_action
            elif spec.direction_anchor_mode == "baseline_direction_only_keep_live_norm":
                mode_action = baseline_direction * live_norm
            elif spec.direction_anchor_mode == "baseline_direction_mix_half_keep_live_norm":
                mode_action = _normalized_halfway_direction(live_direction, baseline_direction) * live_norm
            elif spec.direction_anchor_mode == "local_field_direction_only_keep_live_norm":
                mode_action = local_field_direction * live_norm
            elif spec.direction_anchor_mode == "local_field_direction_mix_half_keep_live_norm":
                mode_action = _normalized_halfway_direction(live_direction, local_field_direction) * live_norm
            ...
            z_lf_next = live_plan.z_lf_next.copy()
            terminal_metrics = _direction_anchor_metrics(
    ...
    if terminal_metrics is None:
        terminal_metrics = {
            ...
            "terminal_post_action_state_slot_displacement": 0.0,
...
def _finalize_cell(
    ...
    *,
    ...
    summary = {
        "phase": "FMPC Stage 04 Incremental Bridge",
        "stage": "terminal_direction_anchor_diagnostic",
        "diagnostic_only": True,
...
def _aggregate_direction_anchor_drift(
    direction_rows: list[dict[str, Any]],
    *,
    mode: TerminalDirectionAnchorMode,
) -> dict[str, float | None]:
    relevant = [row for row in direction_rows if row["handoff_mode"] == mode]
    if not relevant:
        return {
            ...
            "mean_terminal_post_action_state_slot_displacement": None,
    ...
    return {
        ...
        "mean_terminal_post_action_state_slot_displacement": _mean(
            [float(row["terminal_post_action_state_slot_displacement"]) for row in relevant]
        ),
...
def _run_one_seed(
    ...
    *,
    ...
    cells: dict[tuple[str, TerminalDirectionAnchorMode], _CellRuntime] = {}
    ...
    for epoch_index in range(int(baseline_control.config.epochs)):
        ...
        epoch_metric_lists: dict[tuple[str, TerminalDirectionAnchorMode], dict[str, list[float]]] = {
            cell_key: {
                "train_loss": [],
                "train_boot_loss": [],
                "train_identity_loss": [],
        ...
        epoch_direction_metric_lists: dict[TerminalDirectionAnchorMode, dict[str, list[float]]] = {
            spec.mode: {
                ...
                "terminal_post_action_state_slot_displacement": [],
        ...
        for x_batch, y_batch in fmpc_tf2_module.iter_minibatches(
            ...
            for spec in _cell_specs():
                ...
                train_loss, boot_loss, identity_loss, transported_energy, direction_metrics = (
                    _train_one_batch_terminal_direction_anchor(
                        cell.model,
                        cell.psi_network,
                        cell.config,
                        x_batch,
    ...
    return artifacts, direction_drift_rows


def run_fmpc_tf2_terminal_direction_anchor_suite(
    config: FMPCTF2TerminalDirectionAnchorSuiteConfig,
) -> FMPCTF2TerminalDirectionAnchorSuiteRunResult:
    ...
    for seed in config.seeds:
        artifacts, seed_direction_rows = _run_one_seed(
            run_dir,
            config,
        ...
        direction_drift_rows.extend(seed_direction_rows)

    _write_csv(run_dir / "end_to_end_runs.csv", run_rows)
    _write_csv(run_dir / "terminal_direction_anchor_drift_epoch_metrics.csv", direction_drift_rows)
    ...
    challenger_modes: list[TerminalDirectionAnchorMode] = [
        "terminal_live_action_raw",
        "terminal_baseline_direction_only_keep_live_norm",
        "terminal_baseline_direction_mix_half_keep_live_norm",
        "terminal_local_field_direction_only_keep_live_norm",
    ...
    terminal_direction_anchor_drift_summary = {
        mode: _aggregate_direction_anchor_drift(direction_drift_rows, mode=mode)
        for mode in challenger_modes
    }
    ...
    pairwise_vs_action_vector = {
        mode: _pairwise_vs_reference(
            run_rows,
            candidate_key="residualized_local_field_poly_rt2",
            mode=mode,
            reference_candidate_key="residualized_local_field_poly_rt2",
            reference_mode="terminal_replay_action_vector_only",
        )
        for mode in challenger_modes
    }
    pairwise_vs_k4 = {
    ...
    def _rescues_against_live_raw(mode: TerminalDirectionAnchorMode) -> bool:
        same_family = pairwise_vs_terminal_live_raw[mode]
        vs_vector = pairwise_vs_action_vector[mode]
        return (
            float(same_family["mean_test_accuracy_delta"]) >= material
            and abs(float(vs_vector["mean_test_accuracy_delta"])) <= tolerance
        )

    def _nearly_matches_baseline_direction_anchor(mode: TerminalDirectionAnchorMode) -> bool:
        comparison = _pairwise_vs_reference(
            run_rows,
            candidate_key="residualized_local_field_poly_rt2",
            mode=mode,
            reference_candidate_key="residualized_local_field_poly_rt2",
            reference_mode="terminal_baseline_direction_only_keep_live_norm",
        )
        return abs(float(comparison["mean_test_accuracy_delta"])) <= tolerance

    small_baseline_anchor_rescue = _rescues_against_live_raw("terminal_baseline_direction_mix_half_keep_live_norm")
    strong_baseline_anchor_rescue = _rescues_against_live_raw("terminal_baseline_direction_only_keep_live_norm")
    vector_replay_rescue = _rescues_against_live_raw("terminal_replay_action_vector_only")
    local_field_full_near = _nearly_matches_baseline_direction_anchor(
        "terminal_local_field_direction_only_keep_live_norm"
    )
    local_field_half_near = _nearly_matches_baseline_direction_anchor(
        "terminal_local_field_direction_mix_half_keep_live_norm"
    )
    local_field_viable = bool(local_field_full_near or local_field_half_near)

    if small_baseline_anchor_rescue and local_field_viable:
        dominant_interpretation = "small_terminal_direction_anchor_is_sufficient_and_teacher_free_local_field_anchor_is_viable"
        next_single_narrow_move = (
            "run one narrow teacher-free terminal direction regularization diagnostic that keeps the live norm "
            "but softly anchors only the final-step action direction toward the local-field surrogate"
        )
    elif small_baseline_anchor_rescue:
        dominant_interpretation = "small_terminal_direction_anchor_is_sufficient_but_teacher_free_local_field_anchor_is_not_yet_viable"
        next_single_narrow_move = (
            "search for a better teacher-free terminal direction surrogate rather than reopening supervision "
            "bundle decomposition or broader TF2 search"
        )
    elif strong_baseline_anchor_rescue and local_field_viable:
        dominant_interpretation = "stronger_terminal_direction_anchor_is_needed_but_teacher_free_local_field_anchor_is_viable"
        next_single_narrow_move = (
            "run one narrow terminal direction trust-region diagnostic using the local-field direction surrogate "
            "instead of replayed baseline directions"
        )
    elif strong_baseline_anchor_rescue:
        dominant_interpretation = "stronger_terminal_direction_anchor_is_needed_and_teacher_free_local_field_anchor_is_not_yet_viable"
        next_single_narrow_move = (
            "search for a better teacher-free terminal direction surrogate before attempting any mainline-safe "
            "terminal direction stabilization"
        )
    elif vector_replay_rescue:
        dominant_interpretation = "direction_alone_is_not_sufficient_in_practice"
        next_single_narrow_move = (
            "run one narrow terminal trust-region diagnostic that jointly stabilizes the final-step direction "
            "and its live post-action transition without reopening broader TF2 search"
        )
    else:
        dominant_interpretation = "terminal_direction_anchor_signal_is_inconclusive"
        next_single_narrow_move = (
            "recheck the terminal action-output stabilization endpoint before opening any broader stabilization sweep"
        )
    ...
    summary = {
        ...
        "terminal_direction_anchor_drift_summary": terminal_direction_anchor_drift_summary,
        ...
        "pairwise_delta_vs_challenger_terminal_replay_action_vector_only": pairwise_vs_action_vector,
        "pairwise_delta_vs_challenger_k4_open_loop_replay_run": pairwise_vs_k4,
        "is_small_baseline_direction_anchor_sufficient": bool(small_baseline_anchor_rescue),
        "is_strong_baseline_direction_anchor_sufficient": bool(strong_baseline_anchor_rescue),
        "is_teacher_free_local_field_direction_surrogate_viable": bool(local_field_viable),
        ...
        "artifacts": {
            ...
            "terminal_direction_anchor_drift_epoch_metrics_csv": "terminal_direction_anchor_drift_epoch_metrics.csv",
        },
    }
    _write_json(run_dir / "aggregate_summary.json", summary)
    return FMPCTF2TerminalDirectionAnchorSuiteRunResult(
        run_dir=run_dir,
        config=_suite_config_payload(config, candidates),
        run_rows=run_rows,
        direction_drift_rows=direction_drift_rows,
        summary=summary,
    )
