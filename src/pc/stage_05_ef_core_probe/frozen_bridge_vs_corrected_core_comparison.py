...
STAGE05_V3C_FUSED_METHOD_NAME: ComparisonMethodName = (
    "stage05_v3c_fused_trajectory_semigroup_contract"
)
...
STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME: ComparisonMethodName = (
    "stage05_v3c_scaled_continuation_blend_trajectory_contract"
)
STAGE05_V3C_COUPLED_DEFECT_PROJECTION_METHOD_NAME: ComparisonMethodName = (
    "stage05_v3c_coupled_defect_projection_trajectory_contract"
)
STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME: ComparisonMethodName = (
    "stage05_v3c_precision_weighted_continuation_corrector_trajectory_contract"
)
...
@dataclass
class Stage05V2ActiveV3CFusedComparisonConfig:
    """Compare fixed-budget v2, the active refined v3-C reference, and the fused v3-C candidate."""

    experiment_name: str = "stage05_v2_active_v3c_fused_contract_comparison"
    output_root: str | Path = "outputs/stage_05_ef_core_probe"
    run_id: str | None = None
    output_layout: OutputLayout = "single_dir"
    comparison_scope: ComparisonScope = "smoke_only"
    ...
    reuse_stage05_fused_candidate_artifacts: bool = False
    fused_candidate_artifact_root: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v2_active_v3c_fused_contract_comparison/"
        "runs/stage05_v3c_fused_trajectory_semigroup_contract"
    )
    contextual_reference_summary_path: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v2_budget_push_validation_1536_to_3072/aggregate_summary.json"
    ...
    def __post_init__(self) -> None:
        if self.dataset_name != "digits":
            raise ValueError(
                "The Stage 05 v2/active-v3-C/fused comparison currently supports digits only."
            )
        if not self.seeds:
            raise ValueError("seeds must contain at least one seed.")
        if self.batch_size <= 0:
...
@dataclass
class Stage05V2ActiveV3CMidpointReconstructedComparisonConfig:
    """Compare fixed-budget v2, the active refined v3-C reference, and the midpoint-reconstructed candidate."""

    experiment_name: str = "stage05_v2_active_v3c_midpoint_reconstructed_contract_comparison"
    output_root: str | Path = "outputs/stage_05_ef_core_probe"
    run_id: str | None = None
    output_layout: OutputLayout = "single_dir"
    comparison_scope: ComparisonScope = "smoke_only"
    ...
    reuse_stage05_midpoint_reconstructed_candidate_artifacts: bool = False
    midpoint_reconstructed_candidate_artifact_root: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v3c_midpoint_reconstructed_candidate_runs/"
        "runs/stage05_v3c_midpoint_reconstructed_trajectory_contract"
    )
    contextual_reference_summary_path: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v2_budget_push_validation_1536_to_3072/aggregate_summary.json"
...
@dataclass
class Stage05V2ActiveV3CPrecisionWeightedContinuationCorrectorComparisonConfig(
    Stage05V2ActiveV3CEndpointLineContinuationBlendComparisonConfig
):
    """Compare fixed-budget v2, the active refined v3-C reference, and the precision-weighted continuation-corrector candidate."""

    experiment_name: str = (
        "stage05_v2_active_v3c_precision_weighted_continuation_corrector_contract_comparison"
    )
    reuse_stage05_precision_weighted_continuation_corrector_candidate_artifacts: bool = False
    precision_weighted_continuation_corrector_candidate_artifact_root: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v3c_precision_weighted_continuation_corrector_candidate_runs/"
        "runs/stage05_v3c_precision_weighted_continuation_corrector_trajectory_contract"
    )


@dataclass
class Stage05V3CContinuationStrengthDiagnosticConfig(
    Stage05V2ActiveV3CEndpointLineContinuationBlendComparisonConfig
):
    """Run one final single-axis continuation-strength diagnostic inside the narrow v3-C micro-family."""

    experiment_name: str = "stage05_v3c_continuation_strength_diagnostic"
    scaled_continuation_blend_gamma_cont: float = 1.5
    reuse_stage05_scaled_continuation_blend_candidate_artifacts: bool = False
    scaled_continuation_blend_candidate_artifact_root: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v3c_scaled_continuation_blend_candidate_runs/"
        "runs/stage05_v3c_scaled_continuation_blend_trajectory_contract"
    )
    endpoint_line_midpoint_summary_path: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v2_active_v3c_endpoint_line_midpoint_contract_comparison/aggregate_summary.json"
    )
    endpoint_line_continuation_blend_summary_path: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v2_active_v3c_endpoint_line_continuation_blend_contract_comparison/aggregate_summary.json"
    )
    coupled_defect_projection_summary_path: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v2_active_v3c_coupled_defect_projection_contract_comparison/aggregate_summary.json"
    )
    precision_weighted_continuation_corrector_summary_path: str | Path = (
        "outputs/stage_05_ef_core_probe/"
        "stage05_v2_active_v3c_precision_weighted_continuation_corrector_contract_comparison/aggregate_summary.json"
    )

    def __post_init__(self) -> None:
        super().__post_init__()
        if self.scaled_continuation_blend_gamma_cont <= 0.0:
            raise ValueError("scaled_continuation_blend_gamma_cont must be positive.")
...
def _load_stage05_v2_contextual_reference(
    config: (
        ...
        | Stage05V3CContinuationStrengthDiagnosticConfig
...
def _single_candidate_role_mapping(summary: dict[str, Any]) -> tuple[str, str]:
    roles = summary.get("comparison_roles")
    if not isinstance(roles, dict):
        raise ValueError("Comparison summary is missing 'comparison_roles'.")
    candidate_pairs = [
        (str(role_name), str(candidate_name))
        for role_name, candidate_name in roles.items()
        if role_name
        not in {"immediate_control", "active_reference_at_comparison_start"}
        and isinstance(candidate_name, str)
    ]
    if len(candidate_pairs) != 1:
        raise ValueError(
            "Expected exactly one non-control comparison role in the micro-family summary."
        )
    return candidate_pairs[0]


def _stage05_v3c_microfamily_entry_from_summary_path(
    summary_path: str | Path,
) -> dict[str, Any]:
    resolved_path = _resolve_repo_path(summary_path)
    if not resolved_path.exists():
        raise FileNotFoundError(
            f"Missing Stage 05 v3-C micro-family summary at '{resolved_path}'."
        )
    summary = _read_json(resolved_path)
    role_name, candidate_name = _single_candidate_role_mapping(summary)
    by_method = summary.get("by_method")
    if not isinstance(by_method, dict):
        raise ValueError("Micro-family summary is missing 'by_method'.")
    candidate_summary = by_method.get(candidate_name)
    if not isinstance(candidate_summary, dict):
        raise ValueError(
            f"Micro-family summary is missing candidate method '{candidate_name}'."
        )
    pairwise_vs_active = summary.get("pairwise_deltas_vs_active_refined_v3c_reference")
    if not isinstance(pairwise_vs_active, dict):
        raise ValueError(
            "Micro-family summary is missing 'pairwise_deltas_vs_active_refined_v3c_reference'."
        )
    return {
        "source_summary_path": _repo_relative_posix(resolved_path),
        ...
        "configured_step_fixed_point_residual_delta_vs_identity_mean": float(
            candidate_summary["configured_step_fixed_point_residual_delta_vs_identity"][
                "mean"
            ]
        ),
        "delta_vs_active_energy_mean": float(
            pairwise_vs_active["configured_step_energy_delta_vs_identity_delta"]["mean"]
        ),
        ...
        "recommended_next_move": str(summary.get("recommended_next_move", "")),
    }


def _stage05_v3c_microfamily_sort_key(entry: dict[str, Any]) -> tuple[float, float]:
    return (
        float(entry["configured_step_energy_delta_vs_identity_mean"]),
        float(entry["configured_step_fixed_point_residual_delta_vs_identity_mean"]),
    )


def _rank_stage05_v3c_microfamily_entries(
    entries: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    ranked: list[dict[str, Any]] = []
    for position, entry in enumerate(
        sorted(entries, key=_stage05_v3c_microfamily_sort_key),
        start=1,
    ):
        ranked.append(
            {
                **entry,
                "ranking_position": int(position),
            }
        )
    return ranked


def _stage05_v3c_existing_microfamily_entries(
    config: Stage05V3CContinuationStrengthDiagnosticConfig,
) -> list[dict[str, Any]]:
    return [
        _stage05_v3c_microfamily_entry_from_summary_path(
            config.endpoint_line_midpoint_summary_path
        ),
        _stage05_v3c_microfamily_entry_from_summary_path(
            config.endpoint_line_continuation_blend_summary_path
        ),
        _stage05_v3c_microfamily_entry_from_summary_path(
            config.coupled_defect_projection_summary_path
        ),
        _stage05_v3c_microfamily_entry_from_summary_path(
            config.precision_weighted_continuation_corrector_summary_path
        ),
    ]
...
def _stage05_v3c_candidate_config(
    ...
    *,
    ...
    elif config.v3c_candidate_method_name == STAGE05_V3C_FUSED_METHOD_NAME:
        builder = build_stage05_v3c_fused_trajectory_semigroup_contract_config
    elif config.v3c_candidate_method_name == STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME:
        builder = build_stage05_v3c_midpoint_reconstructed_trajectory_contract_config
    else:
        raise ValueError(
...
def _stage05_v3c_variant_config(
    ...
    *,
    ...
    if experiment_name == STAGE05_V3C_METHOD_NAME:
        ...
        builder = build_stage05_v3c_stronger_semigroup_weight_config
    elif experiment_name == STAGE05_V3C_FUSED_METHOD_NAME:
        builder = build_stage05_v3c_fused_trajectory_semigroup_contract_config
    elif experiment_name == STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME:
        builder = build_stage05_v3c_midpoint_reconstructed_trajectory_contract_config
    else:
...
def _stage05_active_v3c_reference_config(
    ...
    *,
    ...
    return build_stage05_v3c_stronger_semigroup_weight_config(
        output_root=output_root,
        experiment_name=STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
        output_layout="run_id_subdir",
        run_id=f"seed_{seed}",
        run_seed=seed,
        data_seed=seed,
...
def _stage05_v3c_fused_candidate_config(
    config: Stage05V2ActiveV3CFusedComparisonConfig,
    *,
    seed: int,
    output_root: Path,
) -> FMPCEFExploratoryProbeConfig:
    return build_stage05_v3c_fused_trajectory_semigroup_contract_config(
        output_root=output_root,
        experiment_name=STAGE05_V3C_FUSED_METHOD_NAME,
        output_layout="run_id_subdir",
        run_id=f"seed_{seed}",
        run_seed=seed,
        data_seed=seed,
...
def _stage05_v3c_midpoint_reconstructed_candidate_config(
...
def _stage05_v3c_scaled_continuation_blend_candidate_config(
    config: Stage05V3CContinuationStrengthDiagnosticConfig,
    *,
    seed: int,
    output_root: Path,
) -> FMPCEFExploratoryProbeConfig:
    return build_stage05_v3c_scaled_continuation_blend_trajectory_contract_config(
        output_root=output_root,
        experiment_name=STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
        output_layout="run_id_subdir",
        run_id=f"seed_{seed}",
        run_seed=seed,
        data_seed=seed,
        ...
        alpha_ramp_epochs=int(config.active_v3c_alpha_ramp_epochs),
        lambda_sg=float(config.active_v3c_lambda_sg),
        continuation_blend_scale=float(config.scaled_continuation_blend_gamma_cont),
    )
...
def _stage05_v3c_precision_weighted_continuation_corrector_candidate_config(
    config: Stage05V2ActiveV3CPrecisionWeightedContinuationCorrectorComparisonConfig,
    *,
    seed: int,
    output_root: Path,
) -> FMPCEFExploratoryProbeConfig:
    return build_stage05_v3c_precision_weighted_continuation_corrector_trajectory_contract_config(
        output_root=output_root,
        experiment_name=STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME,
        output_layout="run_id_subdir",
        run_id=f"seed_{seed}",
        run_seed=seed,
        data_seed=seed,
    ...
    *,
    ...
    return {
        ...
        "endpoint_semigroup_consistency_enabled": bool(
            summary.get("endpoint_semigroup_consistency_enabled", False)
        ),
        "contract_fusion_enabled": bool(summary.get("contract_fusion_enabled", False)),
        "target_reconstruction_enabled": bool(summary.get("target_reconstruction_enabled", False)),
        "midpoint_reconstruction_enabled": bool(
            summary.get("midpoint_reconstruction_enabled", False)
        ),
        "continuation_reevaluated_at_reconstructed_midpoint": bool(
            summary.get("continuation_reevaluated_at_reconstructed_midpoint", False)
        ),
        "main_trajectory_contract_identity": summary.get("main_trajectory_contract_identity"),
        "semigroup_consistency_is_auxiliary_only": summary.get(
            "semigroup_consistency_is_auxiliary_only"
        ),
        "exact_detached_target_barycentric_fusion_enabled": bool(
            summary.get("exact_detached_target_barycentric_fusion_enabled", False)
        ),
        ...
        "lambda_sg": summary.get("lambda_sg"),
        ...
        "mean_midpoint_reconstruction_shift_norm": float(
            summary.get("selected_epoch_mean_midpoint_reconstruction_shift_norm", 0.0)
        ),
        "mean_continuation_reevaluation_shift_norm": float(
            summary.get("selected_epoch_mean_continuation_reevaluation_shift_norm", 0.0)
        ),
        "mean_semigroup_endpoint_closure_residual_norm": float(
            summary.get("selected_epoch_mean_semigroup_endpoint_closure_residual_norm", 0.0)
        ),
        "configured_transport_steps": int(config.transport_steps),
        "one_step_energy_delta_vs_identity": float(val_one_step["energy_delta_vs_identity"]),
        "configured_step_energy_delta_vs_identity": float(val_configured["energy_delta_vs_identity"]),
...
def _load_existing_stage05_core_row(
    *,
    ...
    expected_contract_fusion_enabled: bool | None = None,
    expected_target_reconstruction_enabled: bool | None = None,
    expected_main_trajectory_contract_identity: str | None = None,
) -> dict[str, Any]:
    ...
    if expected_contract_fusion_enabled is not None:
        transport_protocol_matches = transport_protocol_matches and (
            bool(summary.get("contract_fusion_enabled", False))
            == bool(expected_contract_fusion_enabled)
        )
    if expected_target_reconstruction_enabled is not None:
        transport_protocol_matches = transport_protocol_matches and (
            bool(summary.get("target_reconstruction_enabled", False))
            == bool(expected_target_reconstruction_enabled)
        )
    if expected_main_trajectory_contract_identity is not None:
        transport_protocol_matches = transport_protocol_matches and (
            str(summary.get("main_trajectory_contract_identity"))
            == str(expected_main_trajectory_contract_identity)
        )
...
def _method_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for field in (
        "mean_midpoint_reconstruction_shift_norm",
        "mean_continuation_reevaluation_shift_norm",
        "mean_semigroup_endpoint_closure_residual_norm",
    ):
        if field in rows[0]:
            summary[field] = _metric_summary(field)
    return summary
...
def _stage05_v2_active_v3c_fused_protocol_payload(
    config: Stage05V2ActiveV3CFusedComparisonConfig,
) -> dict[str, Any]:
    if config.comparison_scope == "smoke_only":
        decision_rule = {
            "purpose": "smoke_ready_v3c_fused_contract_check",
            "task_accuracy_is_report_only": True,
            "artifact_checks_required": True,
            "one_step_mechanism_should_remain_positive": True,
            "configured_step_mechanism_should_remain_positive": True,
        ...
        decision_rule = {
            "purpose": "fixed_budget_v2_vs_active_v3c_vs_fused_contract_comparison",
            "primary_split": "validation",
            "task_accuracy_is_report_only": True,
            "configured_step_improvement_fraction_threshold": float(
                config.configured_step_improvement_fraction_threshold
            ...
            "reuse_stage05_fused_candidate_artifacts": bool(
                config.reuse_stage05_fused_candidate_artifacts
            ),
        }
    return {
        "comparison_scope": str(config.comparison_scope),
        ...
        "stage_05_active_v3c_reference": {
            ...
            "contract_fusion_enabled": False,
            "trajectory_curriculum_schedule_identity": "warmup_sigmoid_to_alpha_floor",
            "main_trajectory_contract_identity": "stacked_trajectory_curriculum_plus_auxiliary_semigroup_probe",
        ...
        "stage_05_fused_candidate": {
            "method_name": STAGE05_V3C_FUSED_METHOD_NAME,
            "candidate_name": STAGE05_V3C_FUSED_METHOD_NAME,
            "transport_family": "two_branch_residual_meanflow_core",
            "explicit_transport_drift_decomposition_enabled": True,
            "trajectory_curriculum_enabled": True,
            "endpoint_semigroup_consistency_enabled": True,
            "contract_fusion_enabled": True,
            "trajectory_curriculum_schedule_identity": "warmup_sigmoid_to_alpha_floor",
            "main_trajectory_contract_identity": "exact_detached_target_barycentric_fusion",
            "semigroup_consistency_is_auxiliary_only": False,
            "exact_detached_target_barycentric_fusion_enabled": True,
            "alpha_floor": float(config.active_v3c_alpha_floor),
            "lambda_traj_curr": float(config.active_v3c_lambda_traj_curr),
            "lambda_sg": float(config.active_v3c_lambda_sg),
            "reference_reused_from_existing_artifacts": bool(
            ...
            "source_artifact_root": (
                _repo_relative_posix(_resolve_repo_path(config.fused_candidate_artifact_root))
                if config.reuse_stage05_fused_candidate_artifacts
                else None
            ),
            "configured_transport_steps": int(config.stage05_transport_steps),
            "epochs": int(config.stage05_epochs),
...
def _stage05_v2_active_v3c_fused_decision(
    *,
    rows: list[dict[str, Any]],
    config: Stage05V2ActiveV3CFusedComparisonConfig,
    by_method: dict[str, dict[str, Any]],
    pairwise_active_v3c_vs_v2: dict[str, Any],
    pairwise_fused_vs_v2: dict[str, Any],
    pairwise_fused_vs_active_v3c: dict[str, Any],
    contextual_reference: dict[str, Any] | None,
) -> tuple[dict[str, Any], str]:
    fused_rows = _method_rows(rows, STAGE05_V3C_FUSED_METHOD_NAME)
    artifact_pass = all(bool(row["deterministic_artifact_checks_passed"]) for row in fused_rows)
    one_step_positive = all(
        float(row["one_step_energy_delta_vs_identity"]) < 0.0 for row in fused_rows
    )
    configured_step_positive = all(bool(row["mechanism_signal_positive"]) for row in fused_rows)

    active_summary = by_method[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME]
    fused_summary = by_method[STAGE05_V3C_FUSED_METHOD_NAME]
    energy_gain_fraction_vs_active = float(
        _negative_magnitude_relative_gain(
            ...
            candidate_value=float(
                fused_summary["configured_step_energy_delta_vs_identity"]["mean"]
            ),
        )
    )
    residual_gain_fraction_vs_active = float(
        _negative_magnitude_relative_gain(
            ...
            candidate_value=float(
                fused_summary["configured_step_fixed_point_residual_delta_vs_identity"]["mean"]
            ),
        )
    )
    ...
    val_accuracy_delta = float(pairwise_fused_vs_active_v3c["val_accuracy_delta"]["mean"])
    test_accuracy_delta = float(pairwise_fused_vs_active_v3c["test_accuracy_delta"]["mean"])
    ...
    if contextual_reference is not None:
        ...
        gap_closure_payload["fused_v3c"] = dict(gap_closure_payload[STAGE05_V3C_FUSED_METHOD_NAME])
        gap_closure_payload["fused_minus_active_v3c"] = {
            "configured_step_energy": float(
                gap_closure_payload[STAGE05_V3C_FUSED_METHOD_NAME]["configured_step_energy"]
                - gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME][
                    "configured_step_energy"
                ]
            ),
            "configured_step_residual": float(
                gap_closure_payload[STAGE05_V3C_FUSED_METHOD_NAME]["configured_step_residual"]
                - gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME][
                    "configured_step_residual"
                ]
            ),
            "val_accuracy": float(
                gap_closure_payload[STAGE05_V3C_FUSED_METHOD_NAME]["val_accuracy"]
                - gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME]["val_accuracy"]
            ),
            "test_accuracy": float(
                gap_closure_payload[STAGE05_V3C_FUSED_METHOD_NAME]["test_accuracy"]
                - gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME]["test_accuracy"]
            ),
        }
        positive_gap_closure = bool(
            gap_closure_payload[STAGE05_V3C_FUSED_METHOD_NAME]["configured_step_energy"]
            ...
            and gap_closure_payload[STAGE05_V3C_FUSED_METHOD_NAME]["configured_step_residual"]
            >= gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME][
                "configured_step_residual"
            ]
        )
    ...
    if config.comparison_scope == "smoke_only":
        recommended_next_move = "run_real_fixed_budget_v2_vs_active_v3c_vs_fused_contract_comparison"
        gap_closure_decision = "pending_real_fixed_budget_v2_vs_active_v3c_vs_fused_contract_comparison"
        rationale = (
            "The smoke run only verifies that the fused v3-C candidate is wired, deterministic, "
            "and comparable against the current active refined v3-C reference."
        )
    else:
        if (
            artifact_pass
            ...
            recommended_next_move = "promote_fused_v3c_as_active_reference"
            gap_closure_decision = "positive_gap_closure_signal_vs_active_v3c"
            rationale = (
                "The fused v3-C candidate materially improves configured-step mechanism over the "
                "current active refined v3-C reference, preserves the mechanism-first gate, and "
                "improves contextual gap closure versus the 3072-epoch same-family reference."
            )
        elif (
            artifact_pass
            and one_step_positive
            ...
            rationale = (
                "The fused v3-C candidate moves configured-step mechanism in the right direction "
                "without yet materially and cleanly displacing the current active refined v3-C "
                "reference."
            )
        else:
            ...
            rationale = (
                "The fused v3-C candidate does not yet show a strong enough configured-step gain "
                "over the current active refined v3-C reference."
            )
    ...
    return (
        {
            ...
            "fused_contract_candidate_name": STAGE05_V3C_FUSED_METHOD_NAME,
            "fused_contract_keeps_one_step_mechanism_positive": bool(one_step_positive),
            "fused_contract_keeps_configured_step_mechanism_positive": bool(
                configured_step_positive
            ),
            "fused_contract_materially_beats_active_v3c_reference": bool(
                materially_beats_active
            ),
            "fused_contract_avoids_obvious_report_accuracy_regression": bool(
                avoids_obvious_accuracy_regression
            ),
            "fused_contract_shows_positive_gap_closure_signal_vs_active_v3c": bool(
                positive_gap_closure
            ),
            "fused_contract_replaces_active_v3c_reference": bool(
                recommended_next_move == "promote_fused_v3c_as_active_reference"
            ),
...
def _stage05_v2_active_v3c_fused_supports_lines(summary: dict[str, Any]) -> list[str]:
    lines = [
        "The comparison preserves the Stage 05 mechanism-first contract.",
        ...
        "The fused candidate makes contract fusion explicit instead of leaving semigroup consistency as an auxiliary-only term.",
    ]
    if summary["comparison_scope"] == "smoke_only":
        if bool(summary["deterministic_artifact_checks_all_pass"]):
            lines.append("The fused-contract smoke run passes deterministic artifact checks.")
        return lines
    lines.append(
        (
            "The fused candidate materially improves configured-step mechanism over the current active refined v3-C reference."
            if bool(summary["fused_contract_materially_beats_active_v3c_reference"])
            else "The fused candidate does not yet materially improve configured-step mechanism over the current active refined v3-C reference."
        )
    )
    return lines


def _stage05_v2_active_v3c_fused_does_not_support_lines() -> list[str]:
    return [
        "This comparison does not justify replacing the frozen Stage 04 bridge on main.",
        "This comparison does not promote task accuracy to the Stage 05 gate.",
        "This comparison does not reopen Stage 04 package-internal work.",
...
def _stage05_v2_active_v3c_fused_report_markdown(report: dict[str, Any]) -> str:
    protocol = report["comparison_protocol"]
    decision = report["decision"]
    lines = [
        ...
        f"- seeds: `{protocol['seeds']}`",
        f"- shared batch size: `{protocol['shared_batch_size']}`",
        f"- Stage 05 epochs: `{protocol['stage_05_fused_candidate']['epochs']}`",
        "",
        "## Decision",
        f"- active refined v3-C reference: `{decision['active_v3c_reference_candidate_name']}`",
        f"- fused candidate: `{decision['fused_contract_candidate_name']}`",
        f"- `fused_contract_materially_beats_active_v3c_reference`: `{decision['fused_contract_materially_beats_active_v3c_reference']}`",
        f"- `fused_contract_avoids_obvious_report_accuracy_regression`: `{decision['fused_contract_avoids_obvious_report_accuracy_regression']}`",
        f"- `fused_contract_shows_positive_gap_closure_signal_vs_active_v3c`: `{decision['fused_contract_shows_positive_gap_closure_signal_vs_active_v3c']}`",
        f"- gap_closure_decision: `{decision['gap_closure_decision']}`",
        f"- recommended next move: `{decision['recommended_next_move']}`",
        f"- rationale: `{decision['decision_rationale']}`",
        "",
...
def _stage05_v2_active_v3c_fused_suite_config_payload(
    config: Stage05V2ActiveV3CFusedComparisonConfig,
) -> dict[str, Any]:
    return {
        "phase": "FMPC Stage 05 EF Core Probe",
        "stage": str(config.experiment_name),
        "comparison_protocol": _stage05_v2_active_v3c_fused_protocol_payload(config),
    }


def run_stage05_v2_active_v3c_fused_contract_comparison(
    config: Stage05V2ActiveV3CFusedComparisonConfig,
) -> FrozenBridgeVsCorrectedCoreComparisonRunResult:
    """Run the fixed-budget v2 vs active-v3-C vs fused-contract comparison."""
    ...
    _write_json(run_dir / "config.json", _stage05_v2_active_v3c_fused_suite_config_payload(config))
    ...
    reuse_fused_root = _resolve_repo_path(config.fused_candidate_artifact_root)
    ...
    if config.reuse_stage05_fused_candidate_artifacts and not reuse_fused_root.exists():
        raise FileNotFoundError(
            f"Missing fused Stage 05 v3-C candidate artifacts at '{reuse_fused_root}'."
        )
    ...
    for seed in config.seeds:
        ...
        if config.reuse_stage05_fused_candidate_artifacts:
            rows.append(
                _load_existing_stage05_core_row(
                    run_index=run_index,
                    existing_run_dir=reuse_fused_root / f"seed_{seed}",
                    seed=seed,
                    expected_dataset_name=config.dataset_name,
                    expected_batch_size=int(config.batch_size),
                    expected_shuffle_batches=bool(config.shuffle_batches),
                    method_name=STAGE05_V3C_FUSED_METHOD_NAME,
                    stage_name="FMPC Stage 05 EF Core Probe Fused v3-C Candidate",
                    expected_total_training_epochs=int(config.stage05_epochs),
                    expected_eval_steps=int(config.stage05_eval_steps),
                    expected_layer_dims=config.stage05_layer_dims,
                    expected_transport_steps=int(config.stage05_transport_steps),
            ...
            fused_config = _stage05_v3c_fused_candidate_config(
                config,
                seed=seed,
                output_root=runs_root,
            )
            fused_result = run_fmpc_ef_exploratory_probe(fused_config)
            rows.append(
                _stage05_core_row(
                    run_index=run_index,
                    suite_run_dir=run_dir,
    ...
    by_method = {
        ...
        STAGE05_V3C_FUSED_METHOD_NAME: _method_summary(
            _method_rows(rows, STAGE05_V3C_FUSED_METHOD_NAME)
        ),
    }
    ...
    pairwise_fused_vs_v2 = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_FUSED_METHOD_NAME,
        reference_method=STAGE05_V2_METHOD_NAME,
    )
    pairwise_fused_vs_active_v3c = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_FUSED_METHOD_NAME,
    ...
    decision, decision_rationale = _stage05_v2_active_v3c_fused_decision(
        rows=rows,
        config=config,
        by_method=by_method,
        pairwise_active_v3c_vs_v2=pairwise_active_v3c_vs_v2,
        pairwise_fused_vs_v2=pairwise_fused_vs_v2,
        pairwise_fused_vs_active_v3c=pairwise_fused_vs_active_v3c,
        contextual_reference=contextual_reference,
    )
    ...
    summary = {
        ...
        "comparison_roles": {
            ...
            "fused_candidate": STAGE05_V3C_FUSED_METHOD_NAME,
        },
        "candidate_identities": [
            STAGE05_V2_METHOD_NAME,
            STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
        ...
        "by_method": by_method,
        "configured_step_mechanism_ranking": configured_step_mechanism_ranking,
        "pairwise_deltas_vs_stage05_v2_reference": pairwise_fused_vs_v2,
        "pairwise_deltas_vs_active_refined_v3c_reference": pairwise_fused_vs_active_v3c,
    ...
    report = {
        ...
        "supports": _stage05_v2_active_v3c_fused_supports_lines(summary),
        "does_not_support": _stage05_v2_active_v3c_fused_does_not_support_lines(),
    }
    _write_json(run_dir / "comparison_report.json", report)
    _write_text(run_dir / "comparison_report.md", _stage05_v2_active_v3c_fused_report_markdown(report))

    return FrozenBridgeVsCorrectedCoreComparisonRunResult(
        run_dir=run_dir,
        config=_stage05_v2_active_v3c_fused_suite_config_payload(config),
        aggregate_rows=rows,
        summary=summary,
        comparison_report=report,
    )
...
def _stage05_v2_active_v3c_midpoint_reconstructed_protocol_payload(
    config: Stage05V2ActiveV3CMidpointReconstructedComparisonConfig,
) -> dict[str, Any]:
    if config.comparison_scope == "smoke_only":
        decision_rule = {
            "purpose": "smoke_ready_v3c_midpoint_reconstructed_contract_check",
            "task_accuracy_is_report_only": True,
            "artifact_checks_required": True,
            "one_step_mechanism_should_remain_positive": True,
            "configured_step_mechanism_should_remain_positive": True,
        ...
        decision_rule = {
            "purpose": "fixed_budget_v2_vs_active_v3c_vs_midpoint_reconstructed_contract_comparison",
            "primary_split": "validation",
            "task_accuracy_is_report_only": True,
            "configured_step_improvement_fraction_threshold": float(
                config.configured_step_improvement_fraction_threshold
            ...
            "reuse_stage05_midpoint_reconstructed_candidate_artifacts": bool(
                config.reuse_stage05_midpoint_reconstructed_candidate_artifacts
            ),
        }
    return {
        "comparison_scope": str(config.comparison_scope),
        ...
        "stage_05_midpoint_reconstructed_candidate": {
            "method_name": STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME,
            "candidate_name": STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME,
            "transport_family": "two_branch_residual_meanflow_core",
            "explicit_transport_drift_decomposition_enabled": True,
            "trajectory_curriculum_enabled": True,
            "endpoint_semigroup_consistency_enabled": True,
            ...
            "source_artifact_root": (
                _repo_relative_posix(
                    _resolve_repo_path(
                        config.midpoint_reconstructed_candidate_artifact_root
                    )
                )
                if config.reuse_stage05_midpoint_reconstructed_candidate_artifacts
                else None
            ),
            "configured_transport_steps": int(config.stage05_transport_steps),
            "epochs": int(config.stage05_epochs),
...
def _stage05_v2_active_v3c_midpoint_reconstructed_decision(
    *,
    rows: list[dict[str, Any]],
    config: Stage05V2ActiveV3CMidpointReconstructedComparisonConfig,
    by_method: dict[str, dict[str, Any]],
    pairwise_active_v3c_vs_v2: dict[str, Any],
    pairwise_midpoint_vs_v2: dict[str, Any],
    pairwise_midpoint_vs_active_v3c: dict[str, Any],
    contextual_reference: dict[str, Any] | None,
) -> tuple[dict[str, Any], str]:
    midpoint_rows = _method_rows(rows, STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME)
    artifact_pass = all(bool(row["deterministic_artifact_checks_passed"]) for row in midpoint_rows)
    one_step_positive = all(
        float(row["one_step_energy_delta_vs_identity"]) < 0.0 for row in midpoint_rows
    )
    configured_step_positive = all(bool(row["mechanism_signal_positive"]) for row in midpoint_rows)

    active_summary = by_method[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME]
    midpoint_summary = by_method[STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME]
    energy_gain_fraction_vs_active = float(
        _negative_magnitude_relative_gain(
            current_value=float(
                active_summary["configured_step_energy_delta_vs_identity"]["mean"]
            ...
            candidate_value=float(
                midpoint_summary["configured_step_energy_delta_vs_identity"]["mean"]
            ),
        )
    )
    residual_gain_fraction_vs_active = float(
        _negative_magnitude_relative_gain(
            ...
            candidate_value=float(
                midpoint_summary["configured_step_fixed_point_residual_delta_vs_identity"]["mean"]
            ),
        )
    )
    materially_beats_active = bool(
    ...
    val_accuracy_delta = float(pairwise_midpoint_vs_active_v3c["val_accuracy_delta"]["mean"])
    test_accuracy_delta = float(pairwise_midpoint_vs_active_v3c["test_accuracy_delta"]["mean"])
    max_accuracy_regression = float(max(-val_accuracy_delta, -test_accuracy_delta, 0.0))
    avoids_obvious_accuracy_regression = bool(
        max_accuracy_regression <= float(config.allowed_accuracy_regression_threshold)
    )
    ...
    if contextual_reference is not None:
        ...
        gap_closure_payload["midpoint_reconstructed_v3c"] = dict(
            gap_closure_payload[STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME]
        )
        gap_closure_payload["midpoint_reconstructed_minus_active_v3c"] = {
            "configured_step_energy": float(
                gap_closure_payload[STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME][
                    "configured_step_energy"
                ]
            ...
            "val_accuracy": float(
                gap_closure_payload[STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME]["val_accuracy"]
                - gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME][
                    "val_accuracy"
                ]
            ),
        ...
        positive_gap_closure = bool(
            ...
            and gap_closure_payload[STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME][
                "configured_step_residual"
            ]
            >= gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME][
                "configured_step_residual"
    ...
    if config.comparison_scope == "smoke_only":
        ...
        rationale = (
            "The smoke run only verifies that the midpoint-reconstructed v3-C candidate is wired, "
        ...
        if (
            ...
            recommended_next_move = "promote_midpoint_reconstructed_v3c_as_active_reference"
            gap_closure_decision = "positive_gap_closure_signal_vs_active_v3c"
            rationale = (
                "The midpoint-reconstructed v3-C candidate materially improves configured-step "
            ...
            recommended_next_move = "keep_midpoint_reconstruction_direction_and_refine_implementation"
            gap_closure_decision = (
                "directional_but_not_material_gap_closure_vs_active_v3c"
                if positive_gap_closure
                else "directional_but_not_material_mechanism_gain_vs_active_v3c"
            ...
            rationale = (
                "The midpoint-reconstructed v3-C candidate moves configured-step mechanism in the "
            ...
            gap_closure_decision = "no_positive_gap_closure_signal_vs_active_v3c"
            rationale = (
                "The midpoint-reconstructed v3-C candidate does not yet show a strong enough "
    ...
    return (
        {
            "deterministic_artifact_checks_all_pass": bool(artifact_pass),
            "active_v3c_reference_candidate_name": STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
            "midpoint_reconstructed_contract_candidate_name": (
                STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME
            ),
            "midpoint_reconstructed_contract_keeps_one_step_mechanism_positive": bool(
                one_step_positive
            ),
            "midpoint_reconstructed_contract_keeps_configured_step_mechanism_positive": bool(
                configured_step_positive
            ),
            "midpoint_reconstructed_contract_materially_beats_active_v3c_reference": bool(
                materially_beats_active
            ),
            "midpoint_reconstructed_contract_avoids_obvious_report_accuracy_regression": bool(
                avoids_obvious_accuracy_regression
            ),
            "midpoint_reconstructed_contract_shows_positive_gap_closure_signal_vs_active_v3c": bool(
                positive_gap_closure
            ),
            "midpoint_reconstructed_contract_replaces_active_v3c_reference": bool(
                recommended_next_move == "promote_midpoint_reconstructed_v3c_as_active_reference"
            ),
            "energy_gain_fraction_vs_active_v3c_reference": float(energy_gain_fraction_vs_active),
            "residual_gain_fraction_vs_active_v3c_reference": float(
...
def _stage05_v2_active_v3c_midpoint_reconstructed_supports_lines(
    summary: dict[str, Any],
) -> list[str]:
    lines = [
        "The comparison preserves the Stage 05 mechanism-first contract.",
        "The comparison exposes explicit pairwise deltas versus both the fixed-budget v2 control and the current active refined v3-C reference.",
        "The midpoint-reconstructed candidate internalizes semigroup consistency through internal-knot target reconstruction rather than an auxiliary-only semigroup loss.",
    ]
    if summary["comparison_scope"] == "smoke_only":
        if bool(summary["deterministic_artifact_checks_all_pass"]):
            lines.append(
                "The midpoint-reconstructed contract smoke run passes deterministic artifact checks."
            )
        return lines
    lines.append(
        (
            "The midpoint-reconstructed candidate materially improves configured-step mechanism over the current active refined v3-C reference."
            if bool(summary["midpoint_reconstructed_contract_materially_beats_active_v3c_reference"])
            else "The midpoint-reconstructed candidate does not yet materially improve configured-step mechanism over the current active refined v3-C reference."
        )
    )
    return lines


def _stage05_v2_active_v3c_midpoint_reconstructed_does_not_support_lines() -> list[str]:
    return [
        "This comparison does not justify replacing the frozen Stage 04 bridge on main.",
        "This comparison does not promote task accuracy to the Stage 05 gate.",
        "This comparison does not reopen Stage 04 package-internal work.",
...
def _stage05_v2_active_v3c_midpoint_reconstructed_report_markdown(
    report: dict[str, Any]
) -> str:
    protocol = report["comparison_protocol"]
    decision = report["decision"]
    lines = [
        ...
        f"- seeds: `{protocol['seeds']}`",
        f"- shared batch size: `{protocol['shared_batch_size']}`",
        f"- Stage 05 epochs: `{protocol['stage_05_midpoint_reconstructed_candidate']['epochs']}`",
        "",
        "## Decision",
        f"- active refined v3-C reference: `{decision['active_v3c_reference_candidate_name']}`",
        f"- midpoint-reconstructed candidate: `{decision['midpoint_reconstructed_contract_candidate_name']}`",
        f"- `midpoint_reconstructed_contract_materially_beats_active_v3c_reference`: `{decision['midpoint_reconstructed_contract_materially_beats_active_v3c_reference']}`",
        f"- `midpoint_reconstructed_contract_avoids_obvious_report_accuracy_regression`: `{decision['midpoint_reconstructed_contract_avoids_obvious_report_accuracy_regression']}`",
        f"- `midpoint_reconstructed_contract_shows_positive_gap_closure_signal_vs_active_v3c`: `{decision['midpoint_reconstructed_contract_shows_positive_gap_closure_signal_vs_active_v3c']}`",
        f"- gap_closure_decision: `{decision['gap_closure_decision']}`",
        f"- recommended next move: `{decision['recommended_next_move']}`",
        f"- rationale: `{decision['decision_rationale']}`",
        "",
...
def _stage05_v2_active_v3c_midpoint_reconstructed_suite_config_payload(
    config: Stage05V2ActiveV3CMidpointReconstructedComparisonConfig,
) -> dict[str, Any]:
    return {
        "phase": "FMPC Stage 05 EF Core Probe",
        "stage": str(config.experiment_name),
        "comparison_protocol": _stage05_v2_active_v3c_midpoint_reconstructed_protocol_payload(
            config
        ),
    }


def run_stage05_v2_active_v3c_midpoint_reconstructed_contract_comparison(
    config: Stage05V2ActiveV3CMidpointReconstructedComparisonConfig,
) -> FrozenBridgeVsCorrectedCoreComparisonRunResult:
    """Run the fixed-budget v2 vs active-v3-C vs midpoint-reconstructed comparison."""
    ...
    _write_json(
        run_dir / "config.json",
        _stage05_v2_active_v3c_midpoint_reconstructed_suite_config_payload(config),
    )
    ...
    reuse_v2_root = _resolve_repo_path(config.reference_artifact_root)
    reuse_active_v3c_root = _resolve_repo_path(config.active_v3c_reference_artifact_root)
    reuse_midpoint_root = _resolve_repo_path(config.midpoint_reconstructed_candidate_artifact_root)
    ...
    if (
        config.reuse_stage05_midpoint_reconstructed_candidate_artifacts
        and not reuse_midpoint_root.exists()
    ):
        raise FileNotFoundError(
            "Missing midpoint-reconstructed Stage 05 v3-C candidate artifacts at "
            f"'{reuse_midpoint_root}'."
        )
    ...
    for seed in config.seeds:
        ...
        if config.reuse_stage05_midpoint_reconstructed_candidate_artifacts:
            rows.append(
                _load_existing_stage05_core_row(
                    run_index=run_index,
                    existing_run_dir=reuse_midpoint_root / f"seed_{seed}",
                    seed=seed,
                    expected_dataset_name=config.dataset_name,
                    expected_batch_size=int(config.batch_size),
                    expected_shuffle_batches=bool(config.shuffle_batches),
                    method_name=STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME,
                    stage_name="FMPC Stage 05 EF Core Probe Midpoint-Reconstructed v3-C Candidate",
                    expected_total_training_epochs=int(config.stage05_epochs),
                    expected_eval_steps=int(config.stage05_eval_steps),
                    expected_layer_dims=config.stage05_layer_dims,
                    expected_transport_steps=int(config.stage05_transport_steps),
            ...
            midpoint_config = _stage05_v3c_midpoint_reconstructed_candidate_config(
                config,
                seed=seed,
                output_root=runs_root,
            )
            midpoint_result = run_fmpc_ef_exploratory_probe(midpoint_config)
            rows.append(
                _stage05_core_row(
                    run_index=run_index,
                    suite_run_dir=run_dir,
    ...
    by_method = {
        ...
        STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME: _method_summary(
            _method_rows(rows, STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME)
        ),
    }
    pairwise_active_v3c_vs_v2 = _pairwise_summary(
        rows,
    ...
    pairwise_midpoint_vs_v2 = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME,
        reference_method=STAGE05_V2_METHOD_NAME,
    )
    pairwise_midpoint_vs_active_v3c = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME,
        reference_method=STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
    )
    ...
    decision, decision_rationale = _stage05_v2_active_v3c_midpoint_reconstructed_decision(
        rows=rows,
        config=config,
        by_method=by_method,
        pairwise_active_v3c_vs_v2=pairwise_active_v3c_vs_v2,
        pairwise_midpoint_vs_v2=pairwise_midpoint_vs_v2,
        pairwise_midpoint_vs_active_v3c=pairwise_midpoint_vs_active_v3c,
        contextual_reference=contextual_reference,
    )
    ...
    summary = {
        ...
        "comparison_roles": {
            "immediate_control": STAGE05_V2_METHOD_NAME,
            "active_reference_at_comparison_start": STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
            "midpoint_reconstructed_candidate": STAGE05_V3C_MIDPOINT_RECONSTRUCTED_METHOD_NAME,
        },
        "candidate_identities": [
            STAGE05_V2_METHOD_NAME,
            STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
        ...
        "by_method": by_method,
        "configured_step_mechanism_ranking": configured_step_mechanism_ranking,
        "pairwise_deltas_vs_stage05_v2_reference": pairwise_midpoint_vs_v2,
        "pairwise_deltas_vs_active_refined_v3c_reference": pairwise_midpoint_vs_active_v3c,
        "pairwise_active_refined_v3c_vs_v2": pairwise_active_v3c_vs_v2,
        "contextual_3072_reference": contextual_reference,
        **decision,
        "decision_rationale": decision_rationale,
    ...
    report = {
        ...
        "supports": _stage05_v2_active_v3c_midpoint_reconstructed_supports_lines(summary),
        "does_not_support": _stage05_v2_active_v3c_midpoint_reconstructed_does_not_support_lines(),
    }
    _write_json(run_dir / "comparison_report.json", report)
    _write_text(
        run_dir / "comparison_report.md",
        _stage05_v2_active_v3c_midpoint_reconstructed_report_markdown(report),
    )

    return FrozenBridgeVsCorrectedCoreComparisonRunResult(
        run_dir=run_dir,
        config=_stage05_v2_active_v3c_midpoint_reconstructed_suite_config_payload(config),
        aggregate_rows=rows,
        summary=summary,
        comparison_report=report,
    )
...
def _stage05_v2_active_v3c_precision_weighted_continuation_corrector_protocol_payload(
    config: Stage05V2ActiveV3CPrecisionWeightedContinuationCorrectorComparisonConfig,
) -> dict[str, Any]:
    if config.comparison_scope == "smoke_only":
        decision_rule = {
            "purpose": "smoke_ready_v3c_precision_weighted_continuation_corrector_contract_check",
            "task_accuracy_is_report_only": True,
            "artifact_checks_required": True,
            "one_step_mechanism_should_remain_positive": True,
            "configured_step_mechanism_should_remain_positive": True,
        ...
        decision_rule = {
            "purpose": (
                "fixed_budget_v2_vs_active_v3c_vs_"
            ...
            "reuse_stage05_precision_weighted_continuation_corrector_candidate_artifacts": bool(
                config.reuse_stage05_precision_weighted_continuation_corrector_candidate_artifacts
            ),
        }
    return {
        "comparison_scope": str(config.comparison_scope),
        ...
        "stage_05_precision_weighted_continuation_corrector_candidate": {
            "method_name": STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME,
            "candidate_name": STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME,
            "transport_family": "two_branch_residual_meanflow_core",
            "explicit_transport_drift_decomposition_enabled": True,
            "trajectory_curriculum_enabled": True,
            "endpoint_semigroup_consistency_enabled": True,
            ...
            "continuation_target_blending_enabled": True,
            "endpoint_implied_continuation_target_enabled": True,
            "precision_weighted_continuation_corrector_enabled": True,
            "continuation_map_closed_form_coefficient_enabled": True,
            "continuation_coefficient_identity": "local_continuation_map_closed_form_eta_cont",
            "continuation_reevaluated_at_reconstructed_midpoint": True,
            "trajectory_curriculum_schedule_identity": "warmup_sigmoid_to_alpha_floor",
            "main_trajectory_contract_identity": (
            ...
            "source_artifact_root": (
                _repo_relative_posix(
                    _resolve_repo_path(
                        config.precision_weighted_continuation_corrector_candidate_artifact_root
                    )
                )
                if config.reuse_stage05_precision_weighted_continuation_corrector_candidate_artifacts
                else None
            ),
            "configured_transport_steps": int(config.stage05_transport_steps),
            "epochs": int(config.stage05_epochs),
...
def _stage05_v2_active_v3c_precision_weighted_continuation_corrector_decision(
    *,
    rows: list[dict[str, Any]],
    config: Stage05V2ActiveV3CPrecisionWeightedContinuationCorrectorComparisonConfig,
    by_method: dict[str, dict[str, Any]],
    pairwise_active_v3c_vs_v2: dict[str, Any],
    pairwise_precision_vs_v2: dict[str, Any],
    pairwise_precision_vs_active_v3c: dict[str, Any],
    contextual_reference: dict[str, Any] | None,
) -> tuple[dict[str, Any], str]:
    precision_rows = _method_rows(
        rows,
        STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME,
    )
    artifact_pass = all(
        bool(row["deterministic_artifact_checks_passed"]) for row in precision_rows
    )
    one_step_positive = all(
        float(row["one_step_energy_delta_vs_identity"]) < 0.0
    ...
    configured_step_positive = all(
        bool(row["mechanism_signal_positive"]) for row in precision_rows
    )

    active_summary = by_method[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME]
    precision_summary = by_method[
        STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME
    ]
    energy_gain_fraction_vs_active = float(
        _negative_magnitude_relative_gain(
            ...
            candidate_value=float(
                precision_summary["configured_step_energy_delta_vs_identity"]["mean"]
            ),
        )
    )
    residual_gain_fraction_vs_active = float(
        _negative_magnitude_relative_gain(
            ...
            candidate_value=float(
                precision_summary["configured_step_fixed_point_residual_delta_vs_identity"][
                    "mean"
                ]
            ),
        )
    ...
    val_accuracy_delta = float(pairwise_precision_vs_active_v3c["val_accuracy_delta"]["mean"])
    test_accuracy_delta = float(
        pairwise_precision_vs_active_v3c["test_accuracy_delta"]["mean"]
    )
    max_accuracy_regression = float(max(-val_accuracy_delta, -test_accuracy_delta, 0.0))
    avoids_obvious_accuracy_regression = bool(
        max_accuracy_regression <= float(config.allowed_accuracy_regression_threshold)
    ...
    if contextual_reference is not None:
        ...
        gap_closure_payload["precision_weighted_continuation_corrector_v3c"] = dict(
            gap_closure_payload[
                STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME
            ]
        )
        gap_closure_payload["precision_weighted_continuation_corrector_minus_active_v3c"] = {
            "configured_step_energy": float(
                gap_closure_payload[
                    STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME
                ]["configured_step_energy"]
    ...
    if config.comparison_scope == "smoke_only":
        recommended_next_move = (
            "run_real_fixed_budget_v2_vs_active_v3c_vs_"
            "precision_weighted_continuation_corrector_contract_comparison"
        )
        gap_closure_decision = (
        ...
        rationale = (
            "The smoke run only verifies that the precision-weighted continuation-corrector "
            "v3-C candidate is wired, deterministic, and comparable against the current "
            "active refined v3-C reference."
        )
    else:
        if (
            ...
            recommended_next_move = (
                "promote_precision_weighted_continuation_corrector_v3c_as_active_reference"
            )
            gap_closure_decision = "positive_gap_closure_signal_vs_active_v3c"
            rationale = (
                "The precision-weighted continuation-corrector v3-C candidate materially "
                "improves configured-step mechanism over the current active refined v3-C "
                "reference, preserves the mechanism-first gate, and improves contextual gap closure."
            )
        elif (
            artifact_pass
            and one_step_positive
            ...
            recommended_next_move = (
                "keep_precision_weighted_continuation_corrector_direction_and_refine_implementation"
            )
            gap_closure_decision = (
                "directional_but_not_material_gap_closure_vs_active_v3c"
                if positive_gap_closure
            ...
            rationale = (
                "The precision-weighted continuation-corrector v3-C candidate moves "
                "configured-step mechanism in the right direction, but it does not yet "
                "materially and cleanly displace the current active refined v3-C reference."
            )
        else:
            recommended_next_move = (
                "retain_stage05_v3c_stronger_semigroup_weight_as_active_reference"
            ...
            gap_closure_decision = "no_positive_gap_closure_signal_vs_active_v3c"
            rationale = (
                "The precision-weighted continuation-corrector v3-C candidate does not yet "
                "show a strong enough configured-step gain over the current active refined "
                "v3-C reference."
            )
    ...
    return (
        {
            "deterministic_artifact_checks_all_pass": bool(artifact_pass),
            "active_v3c_reference_candidate_name": STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
            "precision_weighted_continuation_corrector_contract_candidate_name": (
                STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME
            ),
            "precision_weighted_continuation_corrector_contract_keeps_one_step_mechanism_positive": bool(
                one_step_positive
            ),
            "precision_weighted_continuation_corrector_contract_keeps_configured_step_mechanism_positive": bool(
                configured_step_positive
            ),
            "precision_weighted_continuation_corrector_contract_materially_beats_active_v3c_reference": bool(
                materially_beats_active
            ),
            "precision_weighted_continuation_corrector_contract_avoids_obvious_report_accuracy_regression": bool(
                avoids_obvious_accuracy_regression
            ),
            "precision_weighted_continuation_corrector_contract_shows_positive_gap_closure_signal_vs_active_v3c": bool(
                positive_gap_closure
            ),
            "precision_weighted_continuation_corrector_contract_replaces_active_v3c_reference": bool(
                recommended_next_move
                == "promote_precision_weighted_continuation_corrector_v3c_as_active_reference"
            ),
            "energy_gain_fraction_vs_active_v3c_reference": float(energy_gain_fraction_vs_active),
...
def _stage05_v2_active_v3c_precision_weighted_continuation_corrector_supports_lines(
    summary: dict[str, Any],
) -> list[str]:
    return [
        (
            "The precision-weighted continuation corrector materially beats the active refined v3-C reference."
            if summary[
                "precision_weighted_continuation_corrector_contract_materially_beats_active_v3c_reference"
            ]
            else "The precision-weighted continuation corrector remains directional but not material."
        ),
        (
            "The candidate avoids an obvious report-only accuracy regression."
            if summary[
                "precision_weighted_continuation_corrector_contract_avoids_obvious_report_accuracy_regression"
            ]
            else "The candidate does not preserve the current report-only accuracy envelope."
        ),
    ]


def _stage05_v2_active_v3c_precision_weighted_continuation_corrector_does_not_support_lines() -> list[str]:
    return [
        "This pass does not justify reopening Stage 04.",
        "This pass does not justify reframing Stage 05 around semigroup-only structure.",
    ]


def _stage05_v2_active_v3c_precision_weighted_continuation_corrector_report_markdown(
    report: dict[str, Any],
) -> str:
    protocol = report["comparison_protocol"]
    decision = report["decision"]
    lines = [
        ...
        "",
        "## Protocol",
        f"- scope: `{protocol['comparison_scope']}`",
        f"- seeds: `{protocol['seeds']}`",
        f"- dataset: `{protocol['dataset_name']}`",
        f"- Stage 05 epochs: `{protocol['stage_05_active_v3c_reference']['epochs']}`",
        "",
        "## Decision",
        f"- active refined v3-C reference: `{decision['active_v3c_reference_candidate_name']}`",
        f"- precision-weighted continuation-corrector candidate: `{decision['precision_weighted_continuation_corrector_contract_candidate_name']}`",
        f"- `precision_weighted_continuation_corrector_contract_materially_beats_active_v3c_reference`: `{decision['precision_weighted_continuation_corrector_contract_materially_beats_active_v3c_reference']}`",
        f"- `precision_weighted_continuation_corrector_contract_avoids_obvious_report_accuracy_regression`: `{decision['precision_weighted_continuation_corrector_contract_avoids_obvious_report_accuracy_regression']}`",
        f"- `precision_weighted_continuation_corrector_contract_shows_positive_gap_closure_signal_vs_active_v3c`: `{decision['precision_weighted_continuation_corrector_contract_shows_positive_gap_closure_signal_vs_active_v3c']}`",
        f"- `recommended_next_move`: `{decision['recommended_next_move']}`",
        f"- rationale: `{decision['decision_rationale']}`",
        "",
        "## Pairwise Deltas Vs Active v3-C",
        f"- configured-step validation energy delta vs identity delta: `{report['pairwise_deltas_vs_active_refined_v3c_reference']['configured_step_energy_delta_vs_identity_delta']['mean']}`",
...
def _stage05_v2_active_v3c_precision_weighted_continuation_corrector_suite_config_payload(
    config: Stage05V2ActiveV3CPrecisionWeightedContinuationCorrectorComparisonConfig,
) -> dict[str, Any]:
    return {
        "phase": "FMPC Stage 05 EF Core Probe",
        "stage": str(config.experiment_name),
        "comparison_protocol": (
            _stage05_v2_active_v3c_precision_weighted_continuation_corrector_protocol_payload(
                config
            )
        ),
    }
...
def run_stage05_v2_active_v3c_precision_weighted_continuation_corrector_contract_comparison(
    config: Stage05V2ActiveV3CPrecisionWeightedContinuationCorrectorComparisonConfig,
) -> FrozenBridgeVsCorrectedCoreComparisonRunResult:
    """Run the fixed-budget v2 vs active-v3-C vs precision-weighted continuation-corrector comparison."""
    ...
    _write_json(
        run_dir / "config.json",
        _stage05_v2_active_v3c_precision_weighted_continuation_corrector_suite_config_payload(
            config
        ),
    )
    ...
    reuse_v2_root = _resolve_repo_path(config.reference_artifact_root)
    reuse_active_v3c_root = _resolve_repo_path(config.active_v3c_reference_artifact_root)
    reuse_precision_root = _resolve_repo_path(
        config.precision_weighted_continuation_corrector_candidate_artifact_root
    )
    ...
    if (
        config.reuse_stage05_precision_weighted_continuation_corrector_candidate_artifacts
        and not reuse_precision_root.exists()
    ):
        raise FileNotFoundError(
            "Missing precision-weighted continuation-corrector Stage 05 v3-C candidate artifacts at "
            f"'{reuse_precision_root}'."
        )
    ...
    for seed in config.seeds:
        ...
        if config.reuse_stage05_precision_weighted_continuation_corrector_candidate_artifacts:
            rows.append(
                _load_existing_stage05_core_row(
                    run_index=run_index,
                    existing_run_dir=reuse_precision_root / f"seed_{seed}",
                    seed=seed,
                    expected_dataset_name=config.dataset_name,
                    expected_batch_size=int(config.batch_size),
                    expected_shuffle_batches=bool(config.shuffle_batches),
                    method_name=STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME,
                    stage_name=(
                        "FMPC Stage 05 EF Core Probe Precision-Weighted Continuation-Corrector v3-C Candidate"
                    ),
                    expected_total_training_epochs=int(config.stage05_epochs),
                    expected_eval_steps=int(config.stage05_eval_steps),
                    expected_layer_dims=config.stage05_layer_dims,
            ...
            precision_config = _stage05_v3c_precision_weighted_continuation_corrector_candidate_config(
                config,
                seed=seed,
                output_root=runs_root,
            )
            precision_result = run_fmpc_ef_exploratory_probe(precision_config)
            rows.append(
                _stage05_core_row(
                    run_index=run_index,
                    suite_run_dir=run_dir,
    ...
    by_method = {
        ...
        STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME: _method_summary(
            _method_rows(
                rows,
                STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME,
            )
    ...
    pairwise_precision_vs_v2 = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME,
        reference_method=STAGE05_V2_METHOD_NAME,
    )
    pairwise_precision_vs_active_v3c = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME,
        reference_method=STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
    )
    ...
    decision, decision_rationale = (
        _stage05_v2_active_v3c_precision_weighted_continuation_corrector_decision(
            rows=rows,
            config=config,
            by_method=by_method,
            pairwise_active_v3c_vs_v2=pairwise_active_v3c_vs_v2,
            pairwise_precision_vs_v2=pairwise_precision_vs_v2,
            pairwise_precision_vs_active_v3c=pairwise_precision_vs_active_v3c,
            contextual_reference=contextual_reference,
        )
    )
    ...
    summary = {
        ...
        "comparison_roles": {
            "immediate_control": STAGE05_V2_METHOD_NAME,
            "active_reference_at_comparison_start": STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
            "precision_weighted_continuation_corrector_candidate": (
                STAGE05_V3C_PRECISION_WEIGHTED_CONTINUATION_CORRECTOR_METHOD_NAME
            ),
        },
        "candidate_identities": [
        ...
        "by_method": by_method,
        "configured_step_mechanism_ranking": configured_step_mechanism_ranking,
        "pairwise_deltas_vs_stage05_v2_reference": pairwise_precision_vs_v2,
        "pairwise_deltas_vs_active_refined_v3c_reference": pairwise_precision_vs_active_v3c,
        "pairwise_active_refined_v3c_vs_v2": pairwise_active_v3c_vs_v2,
        "contextual_3072_reference": contextual_reference,
        **decision,
        "decision_rationale": decision_rationale,
    ...
    report = {
        ...
        "supports": _stage05_v2_active_v3c_precision_weighted_continuation_corrector_supports_lines(
            summary
        ),
        "does_not_support": _stage05_v2_active_v3c_precision_weighted_continuation_corrector_does_not_support_lines(),
    }
    _write_json(run_dir / "comparison_report.json", report)
    _write_text(
        run_dir / "comparison_report.md",
        _stage05_v2_active_v3c_precision_weighted_continuation_corrector_report_markdown(
            report
        ),
    )
    ...
    return FrozenBridgeVsCorrectedCoreComparisonRunResult(
        run_dir=run_dir,
        config=_stage05_v2_active_v3c_precision_weighted_continuation_corrector_suite_config_payload(
            config
        ),
        aggregate_rows=rows,
        summary=summary,
...
def _stage05_v3c_continuation_strength_diagnostic_protocol_payload(
    config: Stage05V3CContinuationStrengthDiagnosticConfig,
) -> dict[str, Any]:
    if config.comparison_scope == "smoke_only":
        decision_rule = {
            "purpose": "smoke_ready_stage05_v3c_continuation_strength_diagnostic",
            "task_accuracy_is_report_only": True,
            "artifact_checks_required": True,
            "smoke_only": True,
        }
        ...
        decision_rule = {
            "purpose": "final_single_axis_stage05_v3c_continuation_strength_diagnostic",
            "primary_split": "validation",
            "task_accuracy_is_report_only": True,
            "configured_step_improvement_fraction_threshold": float(
                config.configured_step_improvement_fraction_threshold
            ...
            "reuse_stage05_scaled_continuation_blend_candidate_artifacts": bool(
                config.reuse_stage05_scaled_continuation_blend_candidate_artifacts
            ),
        }
    return {
        "comparison_scope": str(config.comparison_scope),
        ...
        "stage_05_endpoint_line_continuation_blend_predecessor": {
            "method_name": STAGE05_V3C_ENDPOINT_LINE_CONTINUATION_BLEND_METHOD_NAME,
            "candidate_name": STAGE05_V3C_ENDPOINT_LINE_CONTINUATION_BLEND_METHOD_NAME,
            "transport_family": "two_branch_residual_meanflow_core",
            "explicit_transport_drift_decomposition_enabled": True,
        ...
        "stage_05_scaled_continuation_blend_candidate": {
            "method_name": STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
            "candidate_name": STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
            "transport_family": "two_branch_residual_meanflow_core",
            "explicit_transport_drift_decomposition_enabled": True,
            "trajectory_curriculum_enabled": True,
            "endpoint_semigroup_consistency_enabled": True,
            ...
            "continuation_blend_scale_value": float(
                config.scaled_continuation_blend_gamma_cont
            ),
            ...
            "source_artifact_root": (
                _repo_relative_posix(
                    _resolve_repo_path(
                        config.scaled_continuation_blend_candidate_artifact_root
                    )
                )
                if config.reuse_stage05_scaled_continuation_blend_candidate_artifacts
                else None
            ),
            "configured_transport_steps": int(config.stage05_transport_steps),
            "epochs": int(config.stage05_epochs),
...
def _stage05_v3c_continuation_strength_diagnostic_decision(
    *,
    rows: list[dict[str, Any]],
    config: Stage05V3CContinuationStrengthDiagnosticConfig,
    by_method: dict[str, dict[str, Any]],
    pairwise_scaled_vs_active_v3c: dict[str, Any],
    pairwise_scaled_vs_continuation_blend: dict[str, Any],
    ranked_microfamily_entries: list[dict[str, Any]],
    contextual_reference: dict[str, Any] | None,
) -> tuple[dict[str, Any], str]:
    scaled_rows = _method_rows(rows, STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME)
    artifact_pass = all(
        bool(row["deterministic_artifact_checks_passed"]) for row in scaled_rows
    )
    one_step_positive = all(
        float(row["one_step_energy_delta_vs_identity"]) < 0.0 for row in scaled_rows
    )
    configured_step_positive = all(bool(row["mechanism_signal_positive"]) for row in scaled_rows)

    active_summary = by_method[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME]
    continuation_blend_summary = by_method[
        STAGE05_V3C_ENDPOINT_LINE_CONTINUATION_BLEND_METHOD_NAME
    ]
    scaled_summary = by_method[STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME]
    ...
    energy_gain_fraction_vs_active = float(
        _negative_magnitude_relative_gain(
            ...
            candidate_value=float(
                scaled_summary["configured_step_energy_delta_vs_identity"]["mean"]
            ),
        )
    )
    residual_gain_fraction_vs_active = float(
        _negative_magnitude_relative_gain(
            current_value=float(
                active_summary["configured_step_fixed_point_residual_delta_vs_identity"][
                    "mean"
                ]
            ),
            candidate_value=float(
                scaled_summary["configured_step_fixed_point_residual_delta_vs_identity"][
                    "mean"
                ]
            ),
        )
    ...
    val_accuracy_delta = float(pairwise_scaled_vs_active_v3c["val_accuracy_delta"]["mean"])
    test_accuracy_delta = float(
        pairwise_scaled_vs_active_v3c["test_accuracy_delta"]["mean"]
    )
    max_accuracy_regression = float(max(-val_accuracy_delta, -test_accuracy_delta, 0.0))
    avoids_obvious_accuracy_regression = bool(
        max_accuracy_regression <= float(config.allowed_accuracy_regression_threshold)
    ...
    scaled_beats_continuation_blend_directionally = bool(
        float(scaled_summary["configured_step_energy_delta_vs_identity"]["mean"])
        < float(continuation_blend_summary["configured_step_energy_delta_vs_identity"]["mean"])
        and float(
            scaled_summary["configured_step_fixed_point_residual_delta_vs_identity"]["mean"]
        )
        < float(
            continuation_blend_summary[
                "configured_step_fixed_point_residual_delta_vs_identity"
    ...
    ranking_position = next(
        int(entry["ranking_position"])
        for entry in ranked_microfamily_entries
        if entry["candidate_name"] == STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME
    )
    strongest_microfamily_candidate = bool(ranking_position == 1)
    supports_continuation_strength = bool(
        scaled_beats_continuation_blend_directionally
        and strongest_microfamily_candidate
        and avoids_obvious_accuracy_regression
    )

    gap_closure_payload: dict[str, Any] | None = None
    positive_gap_closure_vs_active = False
    if contextual_reference is not None:
        gap_closure_payload = _stage05_v3c_refinement_contextual_gap_payload(
            by_method=by_method,
            contextual_reference=contextual_reference,
        ...
        gap_closure_payload["scaled_continuation_blend_v3c"] = dict(
            gap_closure_payload[STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME]
        )
        positive_gap_closure_vs_active = bool(
            gap_closure_payload[STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME][
                "configured_step_energy"
            ]
            > gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME][
            ...
            and gap_closure_payload[STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME][
                "configured_step_residual"
            ]
            >= gap_closure_payload[STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME][
                "configured_step_residual"
    ...
    if config.comparison_scope == "smoke_only":
        final_decision = "pending_real_fixed_budget_stage05_v3c_continuation_strength_diagnostic"
        rationale = (
            "The smoke run only verifies that the scaled continuation-blend diagnostic is wired, "
            "deterministic, and reported against the existing narrow v3-C micro-family."
        )
    elif (
        artifact_pass
        and one_step_positive
        ...
        final_decision = "retain_continuation_strength_direction"
        rationale = (
            "The scaled continuation-blend diagnostic materially beats the current active refined "
            "v3-C reference, stays inside the same scaffold, and becomes the strongest narrow "
            "same-family candidate."
        )
    else:
        final_decision = "freeze_narrow_v3c_contract_consolidation_line_as_locally_saturated"
        rationale = (
            "The scaled continuation-blend diagnostic does not materially and cleanly displace the "
            "current active refined v3-C reference. The narrow v3-C contract-consolidation "
            "micro-family should be treated as locally saturated and the next new Stage 05 "
            "mechanism pass should move above this micro-family rather than remain inside it."
        )
    ...
    return (
        {
            "deterministic_artifact_checks_all_pass": bool(artifact_pass),
            "active_v3c_reference_candidate_name": STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
            "local_best_existing_microfamily_predecessor": (
                STAGE05_V3C_ENDPOINT_LINE_CONTINUATION_BLEND_METHOD_NAME
            ),
            "scaled_continuation_blend_contract_candidate_name": (
                STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME
            ),
            "scaled_continuation_blend_contract_keeps_one_step_mechanism_positive": bool(
                one_step_positive
            ),
            "scaled_continuation_blend_contract_keeps_configured_step_mechanism_positive": bool(
                configured_step_positive
            ),
            "scaled_continuation_blend_contract_materially_beats_active_v3c_reference": bool(
                materially_beats_active
            ),
            "scaled_continuation_blend_contract_avoids_obvious_report_accuracy_regression": bool(
                avoids_obvious_accuracy_regression
            ),
            "scaled_continuation_blend_contract_is_strongest_narrow_microfamily_candidate": bool(
                strongest_microfamily_candidate
            ),
            "scaled_continuation_blend_contract_supports_continuation_strength_as_remaining_lever": bool(
                supports_continuation_strength
            ),
            "scaled_continuation_blend_contract_shows_positive_gap_closure_signal_vs_active_v3c": bool(
                positive_gap_closure_vs_active
            ),
            "scaled_continuation_blend_contract_ranking_position_in_microfamily": int(
                ranking_position
            ),
            "scaled_continuation_blend_contract_beats_endpoint_line_continuation_blend_directionally": bool(
                scaled_beats_continuation_blend_directionally
            ),
            "energy_gain_fraction_vs_active_v3c_reference": float(
                energy_gain_fraction_vs_active
            ),
            "residual_gain_fraction_vs_active_v3c_reference": float(
                residual_gain_fraction_vs_active
            ...
            "contextual_gap_closure_fractions_vs_3072_reference": gap_closure_payload,
            "narrow_v3c_contract_consolidation_line_is_locally_saturated": bool(
                final_decision
                == "freeze_narrow_v3c_contract_consolidation_line_as_locally_saturated"
            ),
            "final_decision": str(final_decision),
            "recommended_next_move": str(final_decision),
        },
        rationale,
    )


def _stage05_v3c_continuation_strength_diagnostic_supports_lines(
    summary: dict[str, Any],
) -> list[str]:
    lines = [
        "The diagnostic preserves the Stage 05 mechanism-first gate and keeps Stage 04 frozen.",
        "The diagnostic keeps the existing endpoint-line continuation-blend scaffold and changes only the continuation-side semigroup correction strength.",
        "The diagnostic ranks the narrow v3-C micro-family explicitly instead of trusting any earlier recommended_next_move field.",
    ]
    if summary["comparison_scope"] == "smoke_only":
        if bool(summary["deterministic_artifact_checks_all_pass"]):
            lines.append(
                "The smoke run passes deterministic artifact checks for the scaled continuation-blend diagnostic."
            )
        return lines
    lines.append(
        (
            "The scaled continuation-blend diagnostic materially beats the active refined v3-C reference."
            if bool(
                summary[
                    "scaled_continuation_blend_contract_materially_beats_active_v3c_reference"
                ]
            )
            else "The scaled continuation-blend diagnostic does not materially beat the active refined v3-C reference."
        )
    )
    return lines


def _stage05_v3c_continuation_strength_diagnostic_does_not_support_lines() -> list[str]:
    return [
        "This diagnostic does not justify replacing the frozen Stage 04 bridge on main.",
        "This diagnostic does not promote task accuracy to the Stage 05 gate.",
        "This diagnostic does not justify another narrow midpoint / continuation / coupled variant if the line freezes here.",
    ]


def _stage05_v3c_continuation_strength_diagnostic_report_markdown(
    report: dict[str, Any]
) -> str:
    protocol = report["comparison_protocol"]
    decision = report["decision"]
    ranking = report["microfamily_ranking"]
    lines = [
        ...
        f"- seeds: `{protocol['seeds']}`",
        f"- shared batch size: `{protocol['shared_batch_size']}`",
        f"- Stage 05 epochs: `{protocol['stage_05_scaled_continuation_blend_candidate']['epochs']}`",
        f"- continuation blend scale: `{protocol['stage_05_scaled_continuation_blend_candidate']['continuation_blend_scale_value']}`",
        "",
        "## Decision",
        f"- local best existing predecessor: `{decision['local_best_existing_microfamily_predecessor']}`",
        f"- scaled candidate: `{decision['scaled_continuation_blend_contract_candidate_name']}`",
        f"- `scaled_continuation_blend_contract_materially_beats_active_v3c_reference`: `{decision['scaled_continuation_blend_contract_materially_beats_active_v3c_reference']}`",
        f"- `scaled_continuation_blend_contract_avoids_obvious_report_accuracy_regression`: `{decision['scaled_continuation_blend_contract_avoids_obvious_report_accuracy_regression']}`",
        f"- `scaled_continuation_blend_contract_is_strongest_narrow_microfamily_candidate`: `{decision['scaled_continuation_blend_contract_is_strongest_narrow_microfamily_candidate']}`",
        f"- `scaled_continuation_blend_contract_supports_continuation_strength_as_remaining_lever`: `{decision['scaled_continuation_blend_contract_supports_continuation_strength_as_remaining_lever']}`",
        f"- `narrow_v3c_contract_consolidation_line_is_locally_saturated`: `{decision['narrow_v3c_contract_consolidation_line_is_locally_saturated']}`",
        f"- `final_decision`: `{decision['final_decision']}`",
        f"- rationale: `{decision['decision_rationale']}`",
        "",
        "## Pairwise Deltas Vs Active v3-C",
        f"- energy delta: `{report['pairwise_deltas_vs_active_refined_v3c_reference']['configured_step_energy_delta_vs_identity_delta']['mean']}`",
        f"- residual delta: `{report['pairwise_deltas_vs_active_refined_v3c_reference']['configured_step_fixed_point_residual_delta_vs_identity_delta']['mean']}`",
        "",
        "## Pairwise Deltas Vs Continuation-Blend Predecessor",
        f"- energy delta: `{report['pairwise_deltas_vs_endpoint_line_continuation_blend_reference']['configured_step_energy_delta_vs_identity_delta']['mean']}`",
        f"- residual delta: `{report['pairwise_deltas_vs_endpoint_line_continuation_blend_reference']['configured_step_fixed_point_residual_delta_vs_identity_delta']['mean']}`",
        "",
    ...
    for entry in ranking:
        lines.append(
            "- "
            f"`#{entry['ranking_position']}` `{entry['candidate_name']}` "
            f"(energy `{entry['configured_step_energy_delta_vs_identity_mean']}`, "
            f"residual `{entry['configured_step_fixed_point_residual_delta_vs_identity_mean']}`)"
        )
    return "\n".join(lines)


def _stage05_v3c_continuation_strength_diagnostic_postmortem_markdown(
    *,
    summary: dict[str, Any],
    ranked_microfamily_entries: list[dict[str, Any]],
) -> str:
    lines = [
        ...
        "",
        f"- local best existing predecessor before this pass: `{summary['local_best_existing_microfamily_predecessor']}`",
        f"- new diagnostic candidate: `{summary['scaled_continuation_blend_contract_candidate_name']}`",
        f"- ranking position of new candidate: `{summary['scaled_continuation_blend_contract_ranking_position_in_microfamily']}`",
        f"- materially beats active refined v3-C: `{summary['scaled_continuation_blend_contract_materially_beats_active_v3c_reference']}`",
        f"- supports continuation-strength as remaining lever: `{summary['scaled_continuation_blend_contract_supports_continuation_strength_as_remaining_lever']}`",
        f"- narrow micro-family locally saturated: `{summary['narrow_v3c_contract_consolidation_line_is_locally_saturated']}`",
    ...
    for entry in ranked_microfamily_entries:
        lines.append(
            "- "
            f"`#{entry['ranking_position']}` `{entry['candidate_name']}` "
            f"(delta vs active energy `{entry['delta_vs_active_energy_mean']}`, "
            f"delta vs active residual `{entry['delta_vs_active_residual_mean']}`)"
        )
    if bool(summary["narrow_v3c_contract_consolidation_line_is_locally_saturated"]):
        lines.extend(
            [
                "",
...
def _stage05_v3c_continuation_strength_diagnostic_suite_config_payload(
    config: Stage05V3CContinuationStrengthDiagnosticConfig,
) -> dict[str, Any]:
    return {
        "phase": "FMPC Stage 05 EF Core Probe",
        "stage": str(config.experiment_name),
        "comparison_protocol": _stage05_v3c_continuation_strength_diagnostic_protocol_payload(
            config
        ),
    }


def run_stage05_v3c_continuation_strength_diagnostic(
    config: Stage05V3CContinuationStrengthDiagnosticConfig,
) -> FrozenBridgeVsCorrectedCoreComparisonRunResult:
    """Run the final single-axis continuation-strength diagnostic inside the narrow v3-C micro-family."""

    existing_microfamily_entries = _rank_stage05_v3c_microfamily_entries(
        _stage05_v3c_existing_microfamily_entries(config)
    )
    local_best_existing = existing_microfamily_entries[0]
    if (
        local_best_existing["candidate_name"]
        != STAGE05_V3C_ENDPOINT_LINE_CONTINUATION_BLEND_METHOD_NAME
    ):
        raise ValueError(
            "Direct artifact reading does not confirm "
            "'stage05_v3c_endpoint_line_continuation_blend_trajectory_contract' as the "
            "local best same-family directional predecessor. "
            f"Observed best predecessor: '{local_best_existing['candidate_name']}'."
        )
    ...
    _write_json(
        run_dir / "config.json",
        _stage05_v3c_continuation_strength_diagnostic_suite_config_payload(config),
    )
    ...
    reuse_scaled_root = _resolve_repo_path(
        config.scaled_continuation_blend_candidate_artifact_root
    )
    ...
    if (
        ...
        raise FileNotFoundError(
            "Missing endpoint-line continuation-blend Stage 05 v3-C predecessor artifacts at "
            f"'{reuse_continuation_blend_root}'."
        )
    if (
        config.reuse_stage05_scaled_continuation_blend_candidate_artifacts
        and not reuse_scaled_root.exists()
    ):
        raise FileNotFoundError(
            "Missing scaled continuation-blend Stage 05 v3-C candidate artifacts at "
            f"'{reuse_scaled_root}'."
        )
    ...
    for seed in config.seeds:
        ...
        if config.reuse_stage05_active_v3c_reference_artifacts:
            ...
            active_config = _stage05_active_v3c_reference_config(
                config,
                seed=seed,
                output_root=runs_root,
            )
            active_result = run_fmpc_ef_exploratory_probe(active_config)
            rows.append(
                _stage05_core_row(
                    run_index=run_index,
                    suite_run_dir=run_dir,
        ...
        if config.reuse_stage05_endpoint_line_continuation_blend_candidate_artifacts:
            rows.append(
                _load_existing_stage05_core_row(
                    ...
                    method_name=STAGE05_V3C_ENDPOINT_LINE_CONTINUATION_BLEND_METHOD_NAME,
                    stage_name=(
                        "FMPC Stage 05 EF Core Probe Endpoint-Line Continuation-Blend v3-C Predecessor"
                    ),
                    expected_total_training_epochs=int(config.stage05_epochs),
                    expected_eval_steps=int(config.stage05_eval_steps),
                    expected_layer_dims=config.stage05_layer_dims,
        ...
        if config.reuse_stage05_scaled_continuation_blend_candidate_artifacts:
            rows.append(
                _load_existing_stage05_core_row(
                    run_index=run_index,
                    existing_run_dir=reuse_scaled_root / f"seed_{seed}",
                    seed=seed,
                    expected_dataset_name=config.dataset_name,
                    expected_batch_size=int(config.batch_size),
                    expected_shuffle_batches=bool(config.shuffle_batches),
                    method_name=STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
                    stage_name="FMPC Stage 05 EF Core Probe Scaled Continuation-Blend v3-C Candidate",
                    expected_total_training_epochs=int(config.stage05_epochs),
                    expected_eval_steps=int(config.stage05_eval_steps),
                    expected_layer_dims=config.stage05_layer_dims,
                    expected_transport_steps=int(config.stage05_transport_steps),
            ...
            scaled_config = _stage05_v3c_scaled_continuation_blend_candidate_config(
                config,
                seed=seed,
                output_root=runs_root,
            )
            scaled_result = run_fmpc_ef_exploratory_probe(scaled_config)
            rows.append(
                _stage05_core_row(
                    run_index=run_index,
                    suite_run_dir=run_dir,
    ...
    by_method = {
        ...
        STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME: _method_summary(
            _method_rows(rows, STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME)
        ),
    }
    pairwise_active_v3c_vs_v2 = _pairwise_summary(
        rows,
    ...
    pairwise_scaled_vs_v2 = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
        reference_method=STAGE05_V2_METHOD_NAME,
    )
    pairwise_scaled_vs_active_v3c = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
        reference_method=STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
    )
    pairwise_scaled_vs_continuation_blend = _pairwise_summary(
        rows,
        candidate_method=STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
        reference_method=STAGE05_V3C_ENDPOINT_LINE_CONTINUATION_BLEND_METHOD_NAME,
    )
    contextual_reference = (
        None
        if config.comparison_scope == "smoke_only"
    ...
    current_entry = {
        "source_summary_path": "aggregate_summary.json",
        "stage": str(config.experiment_name),
        "role_name": "scaled_continuation_blend_candidate",
        "candidate_name": STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
        "configured_step_energy_delta_vs_identity_mean": float(
            by_method[STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME][
                "configured_step_energy_delta_vs_identity"
            ]["mean"]
        ),
        "configured_step_fixed_point_residual_delta_vs_identity_mean": float(
        ...
        "delta_vs_active_energy_mean": float(
            pairwise_scaled_vs_active_v3c["configured_step_energy_delta_vs_identity_delta"][
                "mean"
            ]
        ),
        "delta_vs_active_residual_mean": float(
            pairwise_scaled_vs_active_v3c[
                "configured_step_fixed_point_residual_delta_vs_identity_delta"
            ]["mean"]
        ),
        "recommended_next_move": "",
    }
    ranked_microfamily_entries = _rank_stage05_v3c_microfamily_entries(
        [*existing_microfamily_entries, current_entry]
    )
    _write_csv(
        run_dir / "microfamily_ranking.csv",
        [
            {
                "ranking_position": int(entry["ranking_position"]),
                "candidate_name": str(entry["candidate_name"]),
                "stage": str(entry["stage"]),
                "source_summary_path": str(entry["source_summary_path"]),
                "delta_vs_active_energy_mean": float(entry["delta_vs_active_energy_mean"]),
                "delta_vs_active_residual_mean": float(
                    entry["delta_vs_active_residual_mean"]
                ),
                "configured_step_energy_delta_vs_identity_mean": float(
                    entry["configured_step_energy_delta_vs_identity_mean"]
                ),
                "configured_step_fixed_point_residual_delta_vs_identity_mean": float(
                    entry["configured_step_fixed_point_residual_delta_vs_identity_mean"]
                ),
                "recommended_next_move": str(entry["recommended_next_move"]),
            }
            for entry in ranked_microfamily_entries
        ],
    )

    decision, decision_rationale = _stage05_v3c_continuation_strength_diagnostic_decision(
        rows=rows,
        config=config,
        by_method=by_method,
        pairwise_scaled_vs_active_v3c=pairwise_scaled_vs_active_v3c,
        pairwise_scaled_vs_continuation_blend=pairwise_scaled_vs_continuation_blend,
        ranked_microfamily_entries=ranked_microfamily_entries,
        contextual_reference=contextual_reference,
    )
    ...
    summary = {
        ...
        "comparison_roles": {
            "immediate_control": STAGE05_V2_METHOD_NAME,
            "active_reference_at_comparison_start": STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
            "continuation_blend_predecessor": STAGE05_V3C_ENDPOINT_LINE_CONTINUATION_BLEND_METHOD_NAME,
            "scaled_continuation_blend_candidate": STAGE05_V3C_SCALED_CONTINUATION_BLEND_METHOD_NAME,
        },
        "candidate_identities": [
            STAGE05_V2_METHOD_NAME,
            STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME,
        ...
        "by_method": by_method,
        "configured_step_mechanism_ranking": configured_step_mechanism_ranking,
        "pairwise_deltas_vs_stage05_v2_reference": pairwise_scaled_vs_v2,
        "pairwise_deltas_vs_active_refined_v3c_reference": pairwise_scaled_vs_active_v3c,
        "pairwise_deltas_vs_endpoint_line_continuation_blend_reference": (
            pairwise_scaled_vs_continuation_blend
        ),
        "pairwise_active_refined_v3c_vs_v2": pairwise_active_v3c_vs_v2,
        "pairwise_endpoint_line_continuation_blend_vs_v2": pairwise_continuation_blend_vs_v2,
        "pairwise_endpoint_line_continuation_blend_vs_active_refined_v3c_reference": (
            pairwise_continuation_blend_vs_active_v3c
        ),
        "microfamily_ranking": ranked_microfamily_entries,
        "contextual_3072_reference": contextual_reference,
        **decision,
        "decision_rationale": decision_rationale,
        "aggregate_runs_csv_path": "aggregate_runs.csv",
        "microfamily_ranking_csv_path": "microfamily_ranking.csv",
        "comparison_report_json_path": "comparison_report.json",
        "comparison_report_md_path": "comparison_report.md",
        "postmortem_report_md_path": "postmortem_report.md",
    }
    _write_json(run_dir / "aggregate_summary.json", summary)
    _write_text(
        run_dir / "postmortem_report.md",
        _stage05_v3c_continuation_strength_diagnostic_postmortem_markdown(
            summary=summary,
            ranked_microfamily_entries=ranked_microfamily_entries,
        ),
    )
    ...
    report = {
        ...
        "supports": _stage05_v3c_continuation_strength_diagnostic_supports_lines(
            summary
        ),
        "does_not_support": (
            _stage05_v3c_continuation_strength_diagnostic_does_not_support_lines()
        ),
    }
    _write_json(run_dir / "comparison_report.json", report)
    _write_text(
        run_dir / "comparison_report.md",
        _stage05_v3c_continuation_strength_diagnostic_report_markdown(report),
    )

    return FrozenBridgeVsCorrectedCoreComparisonRunResult(
        run_dir=run_dir,
        config=_stage05_v3c_continuation_strength_diagnostic_suite_config_payload(
            config
        ),
        aggregate_rows=rows,
        summary=summary,
...
def _stage05_v3c_refinement_contextual_gap_payload(
    *,
    ...
    if (
        STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME in payload
        and STAGE05_V3C_METHOD_NAME in payload
    ):
    ...
    if (
        STAGE05_V3C_STRONGER_SEMIGROUP_METHOD_NAME in payload
        and STAGE05_V3B_STRONGER_TRAJ_METHOD_NAME in payload
    ):
