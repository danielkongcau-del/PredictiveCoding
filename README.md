...
- Phase 3 is the next active phase
...
- Local output-retention note:
  - the repository currently keeps the Phase 2g matched-search artifacts, the Phase 2g.1 boundary-check artifacts, and the Phase 2g.1-refreshed downstream multiseed and budget-tradeoff artifacts as the locally retained authority outputs
  - older Phase 2 generated outputs may be pruned locally to keep the workspace smaller and can be regenerated by rerunning the corresponding scripts
...
- Phase 3 real-data evaluation is the next active stage
...
- local retention is intentionally lighter than the full historical experiment tree:
  - the currently retained `outputs/` directories are the most recent Phase 2g / Phase 2g.1 authority artifacts
  - earlier generated outputs are considered reproducible artifacts rather than permanent local fixtures
