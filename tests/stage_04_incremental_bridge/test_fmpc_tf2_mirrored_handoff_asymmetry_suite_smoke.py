...
def load_run():
    module = runpy.run_path(str(ROOT / "experiments" / "stage_04_incremental_bridge" / "fmpc_tf2_mirrored_handoff_asymmetry_suite.py"))
    return module["run"]


def test_fmpc_tf2_mirrored_handoff_asymmetry_suite_writes_expected_schema(tmp_path: Path) -> None:
    result = load_run()(
        output_root=tmp_path,
        run_id="tf2_mirrored_handoff_asymmetry_smoke",
        seeds=(0,),
        epochs=2,
        batch_size=64,
        eval_steps=5,
    ...
    assert (run_dir / "stepwise_asymmetry_drift_epoch_metrics.csv").exists()
    assert (run_dir / "aggregate_summary.json").exists()

    run_rows = _read_csv(run_dir / "end_to_end_runs.csv")
    drift_rows = _read_csv(run_dir / "stepwise_asymmetry_drift_epoch_metrics.csv")
    summary = _read_json(run_dir / "aggregate_summary.json")

    assert len(run_rows) == 10
    assert len(drift_rows) == 64

    first_run = run_rows[0]
    assert "handoff_mode" in first_run
    assert "candidate_key" in first_run
    assert "live_step_count" in first_run
    assert "live_control_family" in first_run
    assert "live_mask" in first_run
    ...
    assert "live_step_count" in first_drift
    assert "live_control_family" in first_drift
    ...
    assert "stepwise_asymmetry_summary" in summary
    assert "matched_live_step_asymmetry_summary" in summary
    ...
    assert "pairwise_delta_vs_matched_mirrored_mode" in summary
