...
def load_run():
    module = runpy.run_path(str(ROOT / "experiments" / "stage_04_incremental_bridge" / "fmpc_tf2_target_lag_coupling_suite.py"))
    return module["run"]


def test_fmpc_tf2_target_lag_coupling_suite_writes_expected_schema(tmp_path: Path) -> None:
    result = load_run()(
        output_root=tmp_path,
        run_id="tf2_target_lag_coupling_smoke",
        seeds=(0,),
        epochs=2,
        batch_size=64,
        eval_steps=5,
    ...
    assert (run_dir / "target_drift_epoch_metrics.csv").exists()
    assert (run_dir / "aggregate_summary.json").exists()

    run_rows = _read_csv(run_dir / "end_to_end_runs.csv")
    drift_rows = _read_csv(run_dir / "target_drift_epoch_metrics.csv")
    ...
    assert "target_snapshot_mode" in first_run
    ...
    assert "target_snapshot_mode" in first_drift
    ...
    assert "target_drift_summary" in summary
    assert "pairwise_vs_baseline_live_target" in summary
    assert "pairwise_vs_same_family_live_target" in summary
    assert "is_live_target_timing_drift_present" in summary
    assert "is_challenger_rescued_by_lag1_target_snapshot" in summary
