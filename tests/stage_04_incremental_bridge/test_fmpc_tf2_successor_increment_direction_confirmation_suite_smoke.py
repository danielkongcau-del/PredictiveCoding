...
def load_run():
    module = runpy.run_path(
        str(ROOT / "experiments" / "stage_04_incremental_bridge" / "fmpc_tf2_successor_increment_direction_confirmation_suite.py")
    )
    return module["run"]


def test_fmpc_tf2_successor_increment_direction_confirmation_suite_writes_expected_schema(
    tmp_path: Path,
) -> None:
    result = load_run()(
        output_root=tmp_path,
        run_id="tf2_successor_increment_direction_confirmation_smoke",
        seeds=(0,),
        epochs=2,
        batch_size=64,
        eval_steps=5,
    ...
    assert summary["stage"] == "adopted_package_preterminal_successor_increment_direction_confirmation"
    assert "increment_direction_component_table" in summary
