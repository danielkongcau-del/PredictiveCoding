...
def test_jpc_bridge_probe_writes_expected_artifacts(tmp_path: Path) -> None:
    module = runpy.run_path(str(ROOT / "experiments" / "stage_04_incremental_bridge" / "tf2_jpc_probe.py"))
    run = module["run"]
    ...
    assert config["phase"] == "FMPC Stage 04 Incremental Bridge"
    assert config["stage"] == "ifmpc_bridge_jpc_probe"
    assert summary["phase"] == "FMPC Stage 04 Incremental Bridge"
